...
at a time: **`dict_decoder.go`, `huffman_code.go`, `inflate.go` and
`token.go` are now their own anchored files**, with 87 anchors between
them — the whole decompressor included. The
...
made at compile time by which constructor is called. The three Go files of the
*compressor* proper are still in `mod.rs` and still unanchored: 47 of
the 91 counted names have nothing behind them, and they are all on the
write side.
