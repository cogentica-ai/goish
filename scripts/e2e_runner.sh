...
# and flag_errorhandling_probe by flag_errorhandling_smoke — it exits 7
# on a clean parse and 2 on the ExitOnError cases, all by design, so the
# driver asserts the codes and e2e must not read them as failures.
...
EXCLUDE="${EXCLUDE:-^(hello_query|http_hello|https_serve|spawn_million|spawn_density|preempt_sysmon|lockfree_ring_bench|alloc_hook_bench|segv_diagnostic_smoke|panic_probe_bare|panic_probe_waitgroup|panic_probe_recover|panic_probe_goexit|map_nil_write_probe|flag_errorhandling_probe|flag_definer_panic_probe)$}"
