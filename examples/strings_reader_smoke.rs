// strings_reader_smoke — strings.Reader and strings.Builder against Go.
// (strings/reader.go, strings/builder.go)
//
// Every expectation below is what a real Go 1.25.5 prints: the values
// and the error strings are the output of
// `tools/gen_strings_reader_ref.go` run in `package strings_test` by
// `scripts/goref.sh`.
//
...
fn main() {
    ...
    {
        let mut r = strings::NewReader(gostring::from_bytes(S));
    ...
    {
        let mut ok = true;
        let mut r = strings::NewReader(gostring::from_bytes(b"abc"));
        let mut p = buf(2);
        let (n, e) = r.Read(&mut p);
        ...
        let mut r2 = strings::NewReader(gostring::from_bytes(b""));
        let mut q = buf(4);
    ...
    {
        let mut ok = true;
        let mut r = strings::NewReader(gostring::from_bytes(b"ab"));
        if etext(&r.UnreadByte()) != b"strings.Reader.UnreadByte: at beginning of string".to_vec() {
            ok = false;
        }
        let (c, e) = r.ReadByte();
        if c != b'a' || !e.IsNil() {
    ...
    {
        let mut ok = true;
        let mut r = strings::NewReader(gostring::from_bytes(b"h\xc3\xa9llo"));
        if etext(&r.UnreadRune()) != b"strings.Reader.UnreadRune: at beginning of string".to_vec() {
            ok = false;
        }
        ...
        if etext(&r.UnreadRune())
            != b"strings.Reader.UnreadRune: previous operation was not ReadRune".to_vec()
        {
            ok = false;
        }
        // An invalid sequence is one U+FFFD of width 1.
        let mut r2 = strings::NewReader(gostring::from_bytes(b"\xff\xfe"));
    ...
    //    (offset, whence, position, error text, Len after)
    {
        const SEEKS: [(i64, i64, i64, &str, i64); 9] = [
            ...
            (3, 0, 3, "", 3),
            (100, 0, 100, "", 0),
            (-1, 0, 0, "strings.Reader.Seek: negative position", 4),
            (0, 2, 6, "", 0),
            (-3, 2, 3, "", 3),
            (-100, 2, 0, "strings.Reader.Seek: negative position", 4),
            (2, 1, 4, "", 2),
            (-1, 1, 1, "", 5),
        ];
        let mut ok = true;
        ...
        while i < SEEKS.len() {
            ...
            let mut r = strings::NewReader(gostring::from_bytes(b"abcdef"));
            let mut p = buf(2);
        ...
        if pos != 0 || etext(&e) != b"strings.Reader.Seek: invalid whence".to_vec() {
            ok = false;
        }
        // Seeking past the end is legal; reading there is EOF.
        let mut r2 = strings::NewReader(gostring::from_bytes(b"abc"));
        let _ = r2.Seek(10, 0);
        let mut q = buf(4);
        let (n, e) = r2.Read(&mut q);
    ...
    //    (offset, n, bytes, error text)
    {
        const ATS: [(i64, i64, &[u8], &str); 6] = [
            ...
            (6, 0, b"", "EOF"),
            (7, 0, b"", "EOF"),
            (-1, 0, b"", "strings.Reader.ReadAt: negative offset"),
        ];
        let mut ok = true;
        let mut r = strings::NewReader(gostring::from_bytes(b"abcdef"));
        let mut p2 = buf(2);
    ...
    //    left to give.
    {
        ...
        let mut p = buf(2);
        let _ = r.Read(&mut p);
        let mut sink = goish::bytes::NewBuffer(slice::<byte>::__from_vec(Vec::new()));
        let (n, e) = r.WriteTo(&mut sink);
        if n != 4 || !e.IsNil() || gb(&sink.String()) != b"cdef".to_vec() || r.Len() != 0 {
            ok = false;
        }
        let mut sink2 = goish::bytes::NewBuffer(slice::<byte>::__from_vec(Vec::new()));
    ...
    // 8. Reset re-arms everything, prevRune included.
    {
        let mut r = strings::NewReader(gostring::from_bytes(b"abc"));
        let _ = r.ReadRune();
        r.Reset(gostring::from_bytes(b"xyz"));
        ...
        if len_after_reset == 3
            && size_after_reset == 3
            && unread == b"strings.Reader.UnreadRune: at beginning of string".to_vec()
            && b == b'x'
        {
    ...
    // 9. Builder: Len tracks bytes not runes, WriteRune reports the
    //    encoded width, and an out-of-range rune writes U+FFFD.
    {
        let mut ok = true;
        ...
        let _ = b.WriteString(gostring::from_bytes(b"h\xc3\xa9"));
        let (n, _) = b.Write(goish::convert::bytes("llo"));
        if n != 3 || b.Len() != 6 {
            ok = false;
        }
        let _ = b.WriteByte(b'!');
        let (nr, _) = b.WriteRune(0x4E16);
        if nr != 3 || b.Len() != 10 {
            ok = false;
        }
        let (nr, _) = b.WriteRune(0x110000);
        if nr != 3 {
            ok = false;
        }
        if gb(&b.String()) != b"h\xc3\xa9llo!\xe4\xb8\x96\xef\xbf\xbd".to_vec() {
            ok = false;
        }
        // Reset empties it; Grow reserves without changing Len.
        let mut b2 = strings::Builder::new();
        let _ = b2.WriteString(gostring::from_bytes(b"abc"));
        b2.Reset();
        if b2.Len() != 0 {
            ok = false;
        }
        b2.Grow(64);
        if b2.Len() != 0 || b2.Cap() < 64 {
            ok = false;
        }
        if gb(&b2.String()).len() != 0 {
            ok = false;
        }
        if ok {
            fmt::Println!("[ 9] Builder                   PASS");
        } else {
            fmt::Println!("[ 9] Builder                   FAIL");
            failed += 1;
        }
    }
