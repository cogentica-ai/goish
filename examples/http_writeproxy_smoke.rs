...
fn main() {
    ...
    // 1. Write emits a relative Request-URI (origin form).
    {
        ...
        if werr.IsNil() && first_line_ok {
            Println!("[ 1] Write origin form         PASS");
        } else {
            Println!("[ 1] Write origin form         FAIL line0={}", s);
            failed += 1;
        }
    }
    ...
    // 2. WriteProxy emits an absolute Request-URI.
    {
        ...
        if werr.IsNil() && first_line_ok {
            Println!("[ 2] WriteProxy absolute URI   PASS");
        } else {
            Println!("[ 2] WriteProxy absolute URI   FAIL line0={}", s);
            failed += 1;
        }
    }
    ...
    // 3. WriteProxy still emits the Host header.
    {
        ...
        if strings::Contains(s, string("\r\nHost: example.com\r\n")) {
            Println!("[ 3] WriteProxy Host header    PASS");
        } else {
            Println!("[ 3] WriteProxy Host header    FAIL");
            failed += 1;
        }
    }
