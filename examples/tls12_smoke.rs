...
use goish::crypto::tls::record::{
    decrypt_record, derive_aead_key_material, derive_key_material, derive_master_secret, encrypt_record,
    prf12, DirectionKeys,
};
use goish::fmt;
...
fn main() {
    let tests: &[(&str, testing::TestFn)] = &[
        ...
        ("TestX509ParseCertificateExtractsPublicKey", test_x509_parse_certificate),
        ("TestHandshakeAgainstCannedServer", test_handshake_canned_server),
        ...
        ("TestClientHelloIncludesSupportedGroups", test_client_hello_includes_supported_groups),
        ("TestClientHelloEmptySniWhenNoServerName", test_client_hello_empty_sni),
        ...
        ("TestAeadKeyMaterialOpenSSLCrossCheck", test_aead_key_material_openssl_cross_check),
...
fn test_prf_vector(t: &mut testing::T) {
    let secret: &[u8] = &[
        ...
        0xb1, 0x76, 0x45, 0x23, 0x89, 0x84, 0xe7, 0x00,
    ];
    let seed: &[u8] = &[
    ...
    let expected: &[u8] = &[
        0x5a, 0x60, 0x3d, 0x81, 0x84, 0xb2, 0x74, 0xa8,
        0xb8, 0xed, 0x54, 0x55, 0x11, 0x3f, 0xf2, 0x1c,
        0x1d, 0x6f, 0x19, 0xcb, 0xb7, 0xfd, 0x44, 0x4d,
        0xe0, 0x45, 0xd3, 0x47, 0xd1, 0x73, 0xfc, 0x69,
    ];
...
fn test_master_secret_shape(t: &mut testing::T) {
    ...
    if master.len() != 48 {
        t.Fatal(fmt::Sprintf!("master secret len = %d, want 48", int64(master.len())));
    }
    ...
    if master == master2 {
        t.Fatal(string::from_static("master secret identical for different premaster — PRF not working"));
    }
}
...
fn test_record_mac_failure(t: &mut testing::T) {
    ...
    if dec_err.IsNil() {
        t.Fatal(string::from_static("decrypt_record should have failed on corrupted data"));
    }
...
fn test_client_hello_bytes(t: &mut testing::T) {
    ...
    if msg[0] != 1 {
        t.Fatal(fmt::Sprintf!("ClientHello: msg_type = %d, want 1", int64(msg[0])));
        return;
    }
    ...
    if body_len != expected_body_len {
        t.Fatal(fmt::Sprintf!(
            ...
            int64(body_len), int64(expected_body_len)
        ));
        return;
    }
...
fn test_parse_server_hello(t: &mut testing::T) {
    ...
    if !err.IsNil() {
        t.Fatal(fmt::Sprintf!("parse_server_hello_fragment error: %s", err.Error()));
        return;
    }
    if got_random != server_random_expected {
        t.Fatal(string::from_static("parse_server_hello_fragment: server_random mismatch"));
        return;
    }
    if got_cs != 0x002f {
        t.Fatal(fmt::Sprintf!(
...
fn test_x509_parse_certificate(t: &mut testing::T) {
    ...
    let cert_der: &[u8] = &[
        0x30, 0x82, 0x01, 0x08,  // Certificate SEQUENCE
          0x30, 0x81, 0xb3,       // TBSCertificate SEQUENCE
            ...
            0x30, 0x5c,
              0x30, 0x0d,  // AlgorithmIdentifier (rsaEncryption)
                0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x01,
                0x05, 0x00,
              // BIT STRING containing RSAPublicKey
              0x03, 0x4b, 0x00,
                0x30, 0x48,  // RSAPublicKey SEQUENCE
                  // modulus = 0x00d1db...e3 (65 bytes with leading 0x00)
                  0x02, 0x41, 0x00,
                    0xd1, 0xdb, 0x69, 0x12, 0x13, 0x0c, 0xc7, 0xd0,
                    0x3b, 0xa4, 0xe2, 0x15, 0xe9, 0x72, 0x99, 0x9e,
                    0x7e, 0xf1, 0xfc, 0x7d, 0x39, 0x6e, 0x31, 0x4c,
                    0xf9, 0xd4, 0xe4, 0xe2, 0x97, 0xb9, 0xa7, 0xaf,
                    0xa6, 0xa8, 0x5a, 0x37, 0x22, 0x61, 0x00, 0x41,
                    0xd2, 0x79, 0xf2, 0x1d, 0xa9, 0x00, 0x7d, 0xcb,
                    0x19, 0x86, 0x40, 0x20, 0x64, 0x74, 0x51, 0xcf,
                    0x7f, 0xac, 0x20, 0x12, 0xc6, 0xab, 0x39, 0xe3,
                  // publicExponent = 65537 (0x010001)
                  0x02, 0x03, 0x01, 0x00, 0x01,
          0x30, 0x0d,  // signatureAlgorithm (sha256WithRSA)
            0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01, 0x0b,
            0x05, 0x00,
    ...
    if !err.IsNil() {
        t.Fatal(fmt::Sprintf!("x509::ParseCertificate error: %s", err.Error()));
        return;
    }
...
fn test_handshake_canned_server(t: &mut testing::T) {
    ...
    impl goish::io::Closer for MockConn {
        fn Close(&mut self) -> goish::errors::error { errors::nil }
    }
    impl goish::net::Conn for MockConn {
        ...
        fn Close(&mut self) -> goish::errors::error { errors::nil }
        fn LocalAddr(&self) -> goish::net::TCPAddr { goish::net::TCPAddr { IP: [0,0,0,0], Port: 0 } }
        fn RemoteAddr(&self) -> goish::net::TCPAddr { goish::net::TCPAddr { IP: [0,0,0,0], Port: 0 } }
        fn SetDeadline(&self, _: goish::time::Time) -> goish::errors::error { errors::nil }
        fn SetReadDeadline(&self, _: goish::time::Time) -> goish::errors::error { errors::nil }
        fn SetWriteDeadline(&self, _: goish::time::Time) -> goish::errors::error { errors::nil }
    }
    ...
    // Write 1: ClientHello (record type=22, handshake type=1)
    {
        ...
        if w.len() < 6 || w[5] != 1 {
            t.Fatal(string::from_static("Write[0]: expected ClientHello (msg_type=1)"));
            return;
        }
    }

    if write_count < 2 {
        t.Fatal(string::from_static("canned handshake: only 1 write; expected ClientKeyExchange"));
        return;
    }
    ...
    // Write 2: ClientKeyExchange (record type=22, handshake type=16)
    {
        ...
        if w.len() < 6 || w[5] != 16 {
            t.Fatal(string::from_static("Write[1]: expected ClientKeyExchange (msg_type=16)"));
            return;
        }
        ...
        if w.len() < 11 {
            t.Fatal(string::from_static("Write[1]: ClientKeyExchange too short for 2-byte-length-prefixed PMS"));
            return;
        }
        ...
        if pms_len == 0 {
            t.Fatal(string::from_static("Write[1]: ClientKeyExchange: encrypted PMS length is 0"));
            return;
        }
    ...
    // Write 3: ChangeCipherSpec (record type=20, body=0x01)
    {
        ...
        if w.len() < 6 || w[5] != 1 {
            t.Fatal(string::from_static("Write[2]: ChangeCipherSpec body should be [0x01]"));
        }
    }
...
fn test_x25519_roundtrip(t: &mut testing::T) {
    ...
    for b in shared_ab.iter() { is_zero_ab |= *b; }
    if is_zero_ab == 0 {
        t.Fatal(string::from_static("X25519 roundtrip: shared_ab is all zeros"));
    }
}
...
fn test_aes_gcm_roundtrip(t: &mut testing::T) {
    ...
    if !derr.IsNil() {
        t.Fatal(fmt::Sprintf!("AES-GCM roundtrip Open failed: %s", derr.Error()));
        return;
    }
    ...
    if tamper_err.IsNil() {
        t.Fatal(string::from_static("AES-GCM roundtrip: tampered tag should cause Open to fail"));
    }
}
...
fn test_aead_record_roundtrip(t: &mut testing::T) {
    use goish::crypto::tls::record::{AeadDirectionKeys, encrypt_record_aead, decrypt_record_aead};
    ...
    if !enc_err.IsNil() {
        t.Fatal(fmt::Sprintf!("encrypt_record_aead failed: %s", enc_err.Error()));
        return;
    }

    let wire_v = wire.__into_vec();
    if wire_v.len() < 5 {
        t.Fatal(string::from_static("encrypt_record_aead: wire bytes too short"));
        return;
    }
    ...
    if !dec_err.IsNil() {
        t.Fatal(fmt::Sprintf!("decrypt_record_aead failed: %s", dec_err.Error()));
        return;
    }
    let got = got_s.__into_vec();
...
fn test_client_hello_offers_ecdhe(t: &mut testing::T) {
    ...
    while i + 1 < cs_bytes.len() {
        ...
        if cs == 0xC02F { found_c02f = true; }
        if cs == 0x002F { found_002f = true; }
        i += 2;
    }
    if !found_c02f {
        t.Fatal(string::from_static("ClientHello: 0xC02F (ECDHE-RSA-AES128-GCM-SHA256) not offered"));
    }
    if !found_002f {
        t.Fatal(string::from_static("ClientHello: 0x002F (RSA-AES128-CBC-SHA) not offered"));
    }
}
...
fn test_client_hello_includes_sni(t: &mut testing::T) {
        ...
        if ext_type == 0x0000 {
            ...
            if ext_len != expected_ext_len {
                t.Fatal(fmt::Sprintf!(
                    ...
                    int64(ext_len), int64(expected_ext_len)
                ));
                return;
            }
            ...
            if list_len != expected_list_len {
                t.Fatal(fmt::Sprintf!(
                    ...
                    int64(list_len), int64(expected_list_len)
                ));
                return;
            }
            // name_type must be 0x00 (host_name)
            if ext_data[2] != 0x00 {
                t.Fatal(fmt::Sprintf!("SNI name_type = 0x{:02x}, want 0x00", int64(ext_data[2])));
                return;
            }
            // host_name_len
    ...
    if !found_sni {
        t.Fatal(string::from_static("TestClientHelloIncludesSNI: SNI extension (type=0x0000) not found"));
    }
}
...
fn test_client_hello_includes_supported_groups(t: &mut testing::T) {
    ...
    while pos + 4 <= exts.len() {
        ...
        if ext_type == 0x000a {
            ...
            if pos + ext_len > exts.len() {
                t.Fatal(string::from_static("supported_groups extension data truncated"));
                return;
            }
            let ext_data = &exts[pos..pos + ext_len];
            if ext_data.len() < 4 {
                t.Fatal(string::from_static("supported_groups extension data too short"));
                return;
            }
            ...
            while gi + 1 < groups_bytes.len() {
                ...
                if gid == 0x001d { found_x25519 = true; }
                gi += 2;
            }
            if !found_x25519 {
                t.Fatal(string::from_static("supported_groups: x25519 (0x001d) not present"));
                return;
            }
            found_groups = true;
        }
    ...
    if !found_groups {
        t.Fatal(string::from_static("TestClientHelloIncludesSupportedGroups: extension type 0x000a not found"));
    }
}
...
fn test_prf_tls12_known_answer(t: &mut testing::T) {
    ...
    if &out64[32..] == expected_first32 {
        t.Fatal(string::from_static("PRF: second block identical to first — broken PRF"));
    }
...
fn test_aead_key_material_layout(t: &mut testing::T) {
    ...
    if aead_km.client.enc_key != block[0..16] {
        t.Fatal(string::from_static("AEAD km: client enc_key slice mismatch (want block[0..16])"));
    }
    if aead_km.server.enc_key != block[16..32] {
        t.Fatal(string::from_static("AEAD km: server enc_key slice mismatch (want block[16..32])"));
    }
    if aead_km.client.iv != block[32..36] {
        t.Fatal(string::from_static("AEAD km: client iv slice mismatch (want block[32..36])"));
    }
    if aead_km.server.iv != block[36..40] {
        t.Fatal(string::from_static("AEAD km: server iv slice mismatch (want block[36..40])"));
    }
}
...
fn test_aead_key_material_openssl_cross_check(t: &mut testing::T) {
    let client_random: [u8; 32] = [
        0xda, 0xe4, 0x39, 0xd5, 0x5e, 0x2b, 0xf4, 0xb6,
        0x32, 0x6c, 0xe5, 0x5a, 0x5c, 0xef, 0x57, 0x31,
        0x3d, 0xb9, 0x9f, 0x41, 0x2a, 0xac, 0xc2, 0xeb,
        0x9a, 0x41, 0xa3, 0x29, 0xa8, 0x46, 0x11, 0x3e,
    ];
    let server_random: [u8; 32] = [
        0x1d, 0xaa, 0xd5, 0xc6, 0xf3, 0x2a, 0x6e, 0xd7,
        0x92, 0xa0, 0xb6, 0xba, 0xbb, 0xa9, 0x50, 0x1d,
        0x2b, 0x5e, 0xb3, 0x96, 0xff, 0xff, 0xde, 0x8e,
        0x44, 0x4f, 0x57, 0x4e, 0x47, 0x52, 0x44, 0x01,
    ];
    let master: [u8; 48] = [
        0xf2, 0xc6, 0x09, 0x40, 0xc8, 0xeb, 0xc3, 0x5a,
        0x89, 0x48, 0x66, 0x7e, 0x7e, 0xab, 0x7d, 0xfd,
        0xe6, 0xe8, 0xdb, 0xd6, 0x6f, 0xd9, 0xc1, 0xdf,
        0x06, 0x45, 0x4a, 0xd5, 0x4c, 0x7c, 0x6e, 0x9f,
        0xd0, 0x8c, 0xe1, 0x0b, 0x37, 0xbc, 0xcc, 0xd0,
        0x63, 0xa7, 0x90, 0xef, 0x38, 0xf1, 0xb6, 0x2f,
    ];
    ...
    let expected_server_enc_key: [u8; 16] = [
        0xc4, 0x31, 0x93, 0xc4, 0x66, 0x32, 0x77, 0xb0,
        0x95, 0x4a, 0xef, 0x86, 0xe9, 0x79, 0x93, 0xb1,
    ];
    let expected_server_iv: [u8; 4] = [0x41, 0xef, 0x3f, 0x75];
    let expected_client_enc_key: [u8; 16] = [
        0x45, 0xdf, 0x39, 0x33, 0xbf, 0x03, 0xa6, 0xa6,
        0x52, 0x7e, 0xf5, 0x4e, 0x48, 0x65, 0xfb, 0xfa,
    ];
...
fn test_gcm_open_live_capture(t: &mut testing::T) {
    ...
    let key: [u8; 16] = [
        0xb5, 0x5b, 0x9f, 0xa2, 0x81, 0x13, 0x81, 0x53,
        0x9e, 0xb5, 0x6c, 0x01, 0x4d, 0x47, 0x37, 0xe9,
    ];
    let nonce12: [u8; 12] = [
    ...
    let ct_and_tag: &[u8] = &[
        0x8b, 0x7f, 0xc6, 0xf9, 0xda, 0xca, 0x33, 0xe7,
        0x43, 0xf0, 0x35, 0x86, 0xe8, 0x3b, 0xb9, 0x1c,
        0x3f, 0xa9, 0xc4, 0x5f, 0x10, 0xa0, 0x10, 0x9d,
        0xc1, 0x85, 0x03, 0x05, 0xbd, 0x28, 0x7a, 0x9b,
    ];
    let expected_pt: &[u8] = &[
        0x14, 0x00, 0x00, 0x0c, // Finished msg: type=0x14, length=12
        0xc4, 0x8a, 0x65, 0x28, 0x1d, 0xc5, 0x63, 0xf1,
        0x1c, 0xbc, 0x71, 0x31, // 12-byte verify_data
    ];

    let key_s = slice::<byte>::__from_vec(key.to_vec());
    let (cipher_opt, err) = aes::NewCipher(key_s);
    if !err.IsNil() {
        t.Fatal(fmt::Sprintf!("GCM live: AES NewCipher error: %s", err.Error()));
        return;
    }
...
fn test_gcm_open_live_capture2(t: &mut testing::T) {
    ...
    let key: [u8; 16] = [
        0xaf, 0x6a, 0x5e, 0x25, 0x64, 0x86, 0xa6, 0x13,
        0x79, 0xd8, 0xdd, 0x7e, 0xc8, 0xf9, 0x54, 0x7f,
    ];
    let nonce12: [u8; 12] = [
    ...
    let ct_and_tag: &[u8] = &[
        0x9b, 0xe4, 0x16, 0x7f, 0x6d, 0xa8, 0x29, 0x2e,
        0x62, 0x47, 0x9a, 0xbc, 0xd6, 0x57, 0x85, 0x38,
        0x09, 0x14, 0xc4, 0x7f, 0x8a, 0x32, 0xae, 0x40,
        0x82, 0xaa, 0xad, 0x97, 0x3d, 0x79, 0xd5, 0x55,
    ];
    let expected_pt: &[u8] = &[
        0x14, 0x00, 0x00, 0x0c, // Finished type+length
        0x1f, 0x8d, 0x8b, 0xa4, 0xbf, 0x80, 0x19, 0x81,
        0xfd, 0x05, 0xf9, 0x32,
    ];
    ...
    let (cipher_opt, err) = aes::NewCipher(key_s);
    if !err.IsNil() {
        t.Fatal(fmt::Sprintf!("GCM live2: AES NewCipher error: %s", err.Error()));
        return;
    }
    let (gcm_opt, err) = NewGCM(cipher_opt.unwrap());
    if !err.IsNil() {
