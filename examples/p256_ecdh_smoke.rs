...
extern crate alloc;
use goish::crypto::tls::legacy_p256::{p256_ecdh_compute, p256_keypair_generate};
use goish::fmt;
use goish::syscall;
...
fn main() {
    ...
    let _d_iut: [u8; 32] = [
        0xDC, 0x51, 0xD3, 0x86, 0x6A, 0x15, 0xBA, 0xCD, 0xE3, 0x3D, 0x96, 0xF9, 0x92, 0xFC, 0xA9,
        0x9D, 0xA7, 0xE6, 0xEF, 0x09, 0x34, 0x01, 0xF9, 0xB6, 0x79, 0x7C, 0x8C, 0x2C, 0x04, 0xAB,
        0x9E, 0xCB,
    ];
    // Responder public key (uncompressed):
    // dResponder private key:
    let _d_resp: [u8; 32] = [
        0x57, 0x19, 0x00, 0x82, 0x1E, 0x99, 0xAC, 0x3D, 0x7F, 0xAC, 0xBA, 0xDE, 0x4E, 0xC2, 0x69,
        0x20, 0xA8, 0xE9, 0xC9, 0xD9, 0x9A, 0xA5, 0x8D, 0x4B, 0xEA, 0x44, 0xC0, 0x2E, 0x54, 0x32,
        0xE2, 0xA4,
    ];
    ...
    let q_static_x: [u8; 32] = [
        0x70, 0x0C, 0x48, 0xF7, 0x7F, 0x56, 0x58, 0x4C, 0x5C, 0xC6, 0x32, 0xCA, 0x65, 0x64, 0x0D,
        0xB9, 0x1B, 0x6B, 0xAC, 0xCE, 0x3A, 0x4D, 0xF6, 0xB4, 0x2C, 0xE7, 0xCC, 0x83, 0x88, 0x33,
        0xD2, 0x87,
    ];
    let q_static_y: [u8; 32] = [
        0xDB, 0x71, 0xE5, 0x09, 0xE3, 0xFD, 0x9B, 0x06, 0x0D, 0xDB, 0x20, 0xBA, 0x5C, 0x51, 0xDC,
        0xC5, 0x94, 0x8D, 0x46, 0xFB, 0xF6, 0x40, 0xDF, 0xE0, 0x44, 0x17, 0x82, 0xCA, 0xB8, 0x5F,
        0xA4, 0xAC,
    ];
    ...
    fmt::Println!(fmt::Sprintf!(
        "[test1] shared_secret expected: %02x%02x%02x%02x...",
        z_expected[0] as i64,
        z_expected[1] as i64,
        z_expected[2] as i64,
        z_expected[3] as i64
    ...
    fmt::Println!(fmt::Sprintf!(
        "[test1] shared_secret got:      %02x%02x%02x%02x...",
        z_got[0] as i64,
        z_got[1] as i64,
        z_got[2] as i64,
        z_got[3] as i64
    ...
    if z_got == z_expected {
        fmt::Println!(goish::string(
            "[test1] PASS: NIST CAVP P-256 ECDH vector matches"
        ));
    } else {
        fmt::Println!(goish::string(
            "[test1] FAIL: NIST CAVP P-256 ECDH vector mismatch"
        ));
    ...
    let z_6g_x: [u8; 32] = [
        0xB0, 0x1A, 0x17, 0x2A, 0x76, 0xA4, 0x60, 0x2C, 0x92, 0xD3, 0x24, 0x2C, 0xB8, 0x97, 0xDD,
        0xE3, 0x02, 0x4C, 0x74, 0x0D, 0xEB, 0xB2, 0x15, 0xB4, 0xC6, 0xB0, 0xAA, 0xE9, 0x3C, 0x22,
        0x91, 0xA9,
    ];
    ...
    if z2a == z_6g_x && z2b == z_6g_x {
        fmt::Println!(goish::string(
            "[test2] PASS: small-scalar ECDH (2*3G == 3*2G == 6G)"
        ));
    } else {
        fmt::Println!(goish::string("[test2] FAIL: small-scalar ECDH mismatch"));
        fmt::Println!(fmt::Sprintf!(
            "[test2] 2*3G: %02x%02x%02x%02x... (expect b2eff2f1...)",
            z2a[0] as i64,
            z2a[1] as i64,
            z2a[2] as i64,
            z2a[3] as i64
        ...
        fmt::Println!(fmt::Sprintf!(
            "[test2] 3*2G: %02x%02x%02x%02x... (expect b2eff2f1...)",
            z2b[0] as i64,
            z2b[1] as i64,
            z2b[2] as i64,
            z2b[3] as i64
    ...
    for _trial in 0i64..10i64 {
        ...
        if z_a == z_b {
            ...
            if t3_fail <= 3 {
                fmt::Println!(fmt::Sprintf!(
                    "[test3] FAIL: ECDH(a,B) != ECDH(b,A) at trial %d",
                    _trial
                ));
            }
            pass = false;
    ...
    fmt::Println!(fmt::Sprintf!(
        "[test3] ECDH self-consistency: %d/50 pass",
        t3_pass
    ));
