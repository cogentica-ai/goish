...
use goish::{errors, int, nil, os, rune, string, syscall};
...
fn main() {
    // (1) %s with string arg.
    let s = fmt::Sprintf!("hello %s!", string("world"));
    check(s == "hello world!", b"fmt: %s wrong\n");

    // (2) %d with int arg.
    let s = fmt::Sprintf!("n=%d", 42 as int);
    check(s == "n=42", b"fmt: %d wrong\n");

    // (3) %d with negative int.
    let s = fmt::Sprintf!("n=%d", -7 as int);
    check(s == "n=-7", b"fmt: %d negative wrong\n");

    // (4) %x / %X — hex.
    let s = fmt::Sprintf!("%x %X", 255 as int, 255 as int);
    check(s == "ff FF", b"fmt: %x/%X wrong\n");

    // (5) %b — binary.
    let s = fmt::Sprintf!("%b", 10 as int);
    check(s == "1010", b"fmt: %b wrong\n");

    // (6) %o — octal.
    let s = fmt::Sprintf!("%o", 8 as int);
    check(s == "10", b"fmt: %o wrong\n");

    // (7) %t — bool.
    let s = fmt::Sprintf!("%t %t", true, false);
    check(s == "true false", b"fmt: %t wrong\n");

    // (8) %v — default.
    let s = fmt::Sprintf!("%v %v", 7 as int, string("x"));
    check(s == "7 x", b"fmt: %v wrong\n");

    // (9) %% — literal percent.
    let s = fmt::Sprintf!("100%% done");
    check(s == "100% done", b"fmt: %% wrong\n");

    // (10) %q — quoted string.
    let s = fmt::Sprintf!("%q", string("hi\nthere"));
    check(s == "\"hi\\nthere\"", b"fmt: %q wrong\n");

    // (11) %c — rune as character.
    let s = fmt::Sprintf!("%c", 'A' as rune);
    check(s == "A", b"fmt: %c wrong\n");

    // (12) Multiple args, mixed verbs.
    let s = fmt::Sprintf!("[%d] %s = %v", 1 as int, string("k"), true);
    ...
    // (18) Printf — to stdout.
    fmt::Printf!("count=%d, ok=%t\n", 5 as int, true);

    // (19) Fprintf — to a writer.
    let mut e = os::Stderr();
    fmt::Fprintf!(e, "fmt: fprintf %d to stderr\n", 99 as int);

    // (20) Fprintln — to a writer.
    let mut o = os::Stdout();
    fmt::Fprintln!(o, "fmt: fprintln", 1 as int, 2 as int);
