...
fn main() {
    ...
    check(
        bytes::Compare(b"abc", b"abd") == -1,
        b"bytes: Compare lt wrong\n",
    );
    check(
        bytes::Compare(b"abc", b"abc") == 0,
        b"bytes: Compare eq wrong\n",
    );
    check(
        bytes::Compare(b"abd", b"abc") == 1,
        b"bytes: Compare gt wrong\n",
    );
    ...
    check(
        bytes::Contains(b"foobar", b"oba"),
        b"bytes: Contains hit wrong\n",
    );
    check(
        !bytes::Contains(b"foobar", b"xyz"),
        b"bytes: Contains miss wrong\n",
    );
    check(
        bytes::HasPrefix(b"greetings", b"greet"),
        b"bytes: HasPrefix wrong\n",
    );
    check(
        bytes::HasSuffix(b"greetings", b"ings"),
        b"bytes: HasSuffix wrong\n",
    );
    check(bytes::Index(b"abcdef", b"cd") == 2, b"bytes: Index wrong\n");
    check(
        bytes::IndexByte(b"hello", b'l') == 2,
        b"bytes: IndexByte wrong\n",
    );
    check(
        bytes::LastIndex(b"ababab", b"ab") == 4,
        b"bytes: LastIndex wrong\n",
    );
    check(
        bytes::LastIndexByte(b"hello", b'l') == 3,
        b"bytes: LastIndexByte wrong\n",
    );
    ...
    check(
        bytes::EqualFold(b"Hello", b"hELLO"),
        b"bytes: EqualFold wrong\n",
    );
    ...
    check(
        bytes::Equal(parts[0].clone(), b"a"),
        b"bytes: Split[0] wrong\n",
    );
    check(
        bytes::Equal(parts[1].clone(), b"b"),
        b"bytes: Split[1] wrong\n",
    );
    check(
        bytes::Equal(parts[2].clone(), b"c"),
        b"bytes: Split[2] wrong\n",
    );
    ...
    check(
        bytes::Equal(parts[0].clone(), b"a"),
        b"bytes: SplitN[0] wrong\n",
    );
    check(
        bytes::Equal(parts[1].clone(), b"b,c,d"),
        b"bytes: SplitN[1] wrong\n",
    );
    ...
    check(
        bytes::Equal(&dst, b"hello"),
        b"bytes: Buffer Read content wrong\n",
    );
    ...
    check(
        buf.String() == "seed",
        b"bytes: NewBufferString contents wrong\n",
    );
    ...
    check(
        slices::Equal(&out, &want),
        b"bytes: Reader+Scanner content wrong\n",
    );
    ...
    check(
        bytes::Equal(b"x", b"x"),
        b"bytes: byte-array literal flow wrong\n",
    );
    let owned = bytes::Clone(b"abc");
    check(
        bytes::Equal(owned, b"abc"),
        b"bytes: owned-vs-literal Equal wrong\n",
    );
