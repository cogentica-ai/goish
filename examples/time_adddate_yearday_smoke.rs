...
fn main() {
    ...
    // 1. YearDay Jan 1 → 1.
    {
        ...
        let d = t.YearDay();
        if d == 1 {
            fmt::Println!("[ 1] YearDay Jan 1             PASS");
        } else {
            fmt::Println!("[ 1] YearDay Jan 1             FAIL got=", d);
            failed += 1;
        }
    }
    ...
    // 2. YearDay Dec 31 leap year (2024) → 366.
    {
        ...
        let d = t.YearDay();
        if d == 366 {
            fmt::Println!("[ 2] YearDay Dec 31 leap       PASS");
        } else {
            fmt::Println!("[ 2] YearDay Dec 31 leap       FAIL got=", d);
            failed += 1;
        }
    }
    ...
    // 3. YearDay Dec 31 non-leap (2023) → 365.
    {
        ...
        let d = t.YearDay();
        if d == 365 {
            fmt::Println!("[ 3] YearDay Dec 31 non-leap   PASS");
        } else {
            fmt::Println!("[ 3] YearDay Dec 31 non-leap   FAIL got=", d);
            failed += 1;
        }
    }
    ...
    // 4. YearDay Mar 1 leap year — 31 (Jan) + 29 (Feb) + 1 = 61.
    {
        ...
        let d = t.YearDay();
        if d == 61 {
            fmt::Println!("[ 4] YearDay Mar 1 leap        PASS");
        } else {
            fmt::Println!("[ 4] YearDay Mar 1 leap        FAIL got=", d);
            failed += 1;
        }
    }
    ...
    // 5. YearDay Mar 1 non-leap — 31 + 28 + 1 = 60.
    {
        ...
        let d = t.YearDay();
        if d == 60 {
            fmt::Println!("[ 5] YearDay Mar 1 non-leap    PASS");
        } else {
            fmt::Println!("[ 5] YearDay Mar 1 non-leap    FAIL got=", d);
            failed += 1;
        }
    }
    ...
    // 6. AddDate adds years (zero-month, zero-day).
    {
        ...
        let (y, m, d) = u.Date();
        if y == 2026 && m == 6 && d == 15 {
            fmt::Println!("[ 6] AddDate years             PASS");
        } else {
            fmt::Println!("[ 6] AddDate years             FAIL");
            failed += 1;
        }
    }
    ...
    // 7. AddDate adds months and overflows year.
    {
        ...
        let (y, m, d) = u.Date();
        if y == 2025 && m == 2 && d == 1 {
            fmt::Println!("[ 7] AddDate months overflow   PASS");
        } else {
            fmt::Println!("[ 7] AddDate months overflow   FAIL");
            failed += 1;
        }
    }
    ...
    // 8. AddDate normalizes Oct 31 + 1 month = Dec 1 (per Go doc).
    {
        ...
        let (y, m, d) = u.Date();
        if y == 2024 && m == 12 && d == 1 {
            fmt::Println!("[ 8] AddDate Oct31+1m → Dec 1  PASS");
        } else {
            fmt::Println!("[ 8] AddDate Oct31+1m → Dec 1  FAIL");
            failed += 1;
        }
    }
    ...
    // 9. AddDate adds days, crossing month boundary.
    {
        ...
        let (y, m, d) = u.Date();
        if y == 2024 && m == 2 && d == 4 {
            fmt::Println!("[ 9] AddDate days cross month  PASS");
        } else {
            fmt::Println!("[ 9] AddDate days cross month  FAIL");
            failed += 1;
        }
    }
    ...
    // 10. AddDate negative deltas (subtract).
    {
        ...
        if y == 2022 && m == 12 && d == 31 {
            ...
            fmt::Println!("[10] AddDate negative          FAIL");
            failed += 1;
        }
    }
    ...
    // 11. AddDate preserves clock (hour/min/sec/nsec).
    {
        ...
        if h == 13 && mi == 45 && s == 7 && u.Nanosecond() == 123_456_789 {
            fmt::Println!("[11] AddDate preserves clock   PASS");
        } else {
            fmt::Println!("[11] AddDate preserves clock   FAIL");
            failed += 1;
        }
    }
