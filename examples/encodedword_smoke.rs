...
fn main() {
    ...
    // 1. ASCII pass-through (no encoding needed).
    {
        ...
        if got == string("hello") {
            fmt::Println!("[ 1] QEncode ASCII unchanged PASS");
        } else {
            fmt::Println!("[ 1] QEncode ASCII unchanged FAIL got {}", got);
            failed += 1;
        }
    }
    ...
    // 2. Q-encode UTF-8 with single non-ASCII rune.
    {
        ...
        if got == string("=?UTF-8?q?Caf=C3=A9?=") {
            fmt::Println!("[ 2] QEncode Café           PASS");
        } else {
            fmt::Println!("[ 2] QEncode Café           FAIL got {}", got);
            failed += 1;
        }
    }
    ...
    // 3. B-encode UTF-8.
    {
        ...
        if got == string("=?UTF-8?b?SGVsbG8sIOS4lueVjA==?=") {
            fmt::Println!("[ 3] BEncode Hello,世界      PASS");
        } else {
            fmt::Println!("[ 3] BEncode Hello,世界      FAIL got {}", got);
            failed += 1;
        }
    }
    ...
    // 4. Newline triggers encoding (tab passes through unchanged).
    {
        ...
        if got == string("=?UTF-8?q?a=0Ab?=") {
            fmt::Println!("[ 4] QEncode newline        PASS");
        } else {
            fmt::Println!("[ 4] QEncode newline        FAIL got {}", got);
            failed += 1;
        }
        let got2 = QEncoding.Encode(string("UTF-8"), string("a\tb"));
        if got2 == string("a\tb") {
            fmt::Println!("[ 4b] QEncode tab unchanged PASS");
        } else {
            fmt::Println!("[ 4b] QEncode tab unchanged FAIL got {}", got2);
            failed += 1;
        }
    }
    ...
    // 5. Decode Q-encoded.
    {
        ...
        if err.IsNil() && got == string("Café") {
            fmt::Println!("[ 5] Decode Q Café          PASS");
        } else {
            fmt::Println!("[ 5] Decode Q Café          FAIL");
            failed += 1;
        }
    }
    ...
    // 6. Decode B-encoded UTF-8.
    {
        ...
        if err.IsNil() && got == string("Hello, 世界") {
            fmt::Println!("[ 6] Decode B 世界           PASS");
        } else {
            fmt::Println!("[ 6] Decode B 世界           FAIL");
            failed += 1;
        }
    }
    ...
    // 7. Empty encoded-word body decodes to "".
    {
        ...
        if err.IsNil() && got == string("") {
            fmt::Println!("[ 7] Decode empty body      PASS");
        } else {
            fmt::Println!("[ 7] Decode empty body      FAIL");
            failed += 1;
        }
    }
    ...
    // 8. DecodeHeader mixed plain + encoded-word.
    {
        ...
        if err.IsNil() && got == string("Subject: Café") {
            fmt::Println!("[ 8] DecodeHeader mixed     PASS");
        } else {
            fmt::Println!("[ 8] DecodeHeader mixed     FAIL got {}", got);
            failed += 1;
        }
    }
    ...
    // 9. DecodeHeader: whitespace between two encoded-words is deleted.
    {
        ...
        if err.IsNil() && got == string("Café World") {
            ...
            fmt::Println!("[ 9] DecodeHeader 2 words   FAIL got {}", got);
            failed += 1;
        }
    }
    ...
    // 10. iso-8859-1 charset decode.
    {
        ...
        if err.IsNil() && got == string("Café") {
            fmt::Println!("[10] Decode iso-8859-1      PASS");
        } else {
            fmt::Println!("[10] Decode iso-8859-1      FAIL");
            failed += 1;
        }
    }
    ...
    // 11. Long Q-encoded string splits into multiple words.
    {
        ...
        if got == want {
            fmt::Println!("[11] Long QEncode split     PASS");
        } else {
            fmt::Println!("[11] Long QEncode split     FAIL got {}", got);
            failed += 1;
        }
    }
    ...
    // 12. Round-trip B-encoded long UTF-8 split into multiple words.
    {
        ...
        if err.IsNil() && decoded == input_s {
            fmt::Println!("[12] BEncode round-trip     PASS");
        } else {
            fmt::Println!("[12] BEncode round-trip     FAIL");
            failed += 1;
        }
    }
