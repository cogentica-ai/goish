...
fn run() {
    // A healthy conn is admitted, and admission marks it reused.
    {
        ...
        check(
            "a healthy conn is pooled and marked reused",
            e.IsNil() && pc.isReused(),
            fmt::Sprintf!("%v", e),
        );
    ...
    // Each rejection is a distinct named error.
    {
        ...
        check(
            "keep-alives disabled is rejected by name",
            !e.IsNil() && e.Error() == "http: putIdleConn: keep alives disabled",
            fmt::Sprintf!("%v", e),
        );
    }
    {
        ...
        let e = t.tryPutIdleConn(&conn("a.com"));
        check(
            "a NEGATIVE MaxIdleConnsPerHost means no pooling for that host",
            !e.IsNil() && e.Error() == "http: putIdleConn: keep alives disabled",
            fmt::Sprintf!("%v", e),
        );
    }
    {
        ...
        let e = t.tryPutIdleConn(&pc);
        check(
            "a broken conn is rejected by name",
            !e.IsNil() && e.Error() == "http: putIdleConn: connection is in bad state",
            fmt::Sprintf!("%v", e),
        );
    }
    {
        ...
        check(
            "per-host cap is 2, and it is PER HOST",
            e1.IsNil()
                && e2.IsNil()
                && !e3.IsNil()
                && e3.Error() == "http: putIdleConn: too many idle connections for host"
                && other.IsNil(),
    ...
    }
    {
        ...
        check(
            "CloseIdleConnections closes what was pooled",
            pc.isBroken(),
            string(""),
        );
        ...
        let e = t.tryPutIdleConn(&conn("a.com"));
        check(
            "and newly idle conns are refused afterwards",
            !e.IsNil() && e.Error() == "http: putIdleConn: CloseIdleConnections was called",
            fmt::Sprintf!("%v", e),
        );
    }
    ...
    // ── queueForIdleConn ──
    {
        ...
        check(
            "a waiter is satisfied from the idle pool",
            got && !w.waiting()
                && w.__delivered()
        ...
        check(
            "the delivered conn is removed, so the next waiter queues",
            !t.queueForIdleConn(&w2) && w2.waiting(),
            string(""),
        );
    }
    {
        ...
        check(
            "a broken idle conn is skipped rather than delivered",
            !t.queueForIdleConn(&w) && w.waiting(),
            string(""),
        );
    }
    {
        ...
        let e = t.tryPutIdleConn(&conn("a.com"));
        check(
            "queueForIdleConn clears closeIdle, so pooling resumes",
            e.IsNil(),
            fmt::Sprintf!("%v", e),
        );
    }
    {
        ...
        let w = Arc::new(wantConn::__new());
        check(
            "DisableKeepAlives refuses without consulting the pool",
            !t.queueForIdleConn(&w) && w.waiting(),
            string(""),
        );
    }
    ...
    // ── getConn ── the idle-HIT path, end to end, no dialing
    {
        ...
        let (r0, _) = goish::net::http::NewRequest(
            string("GET"),
            string("http://a.com/"),
            goish::slice::new(),
        );
        ...
        check(
            "getConn returns the pooled conn without dialing",
        ...
        check(
            "a second getConn finds nothing and queues instead",
            got2.is_none() && !e2.IsNil(),
            string(""),
        );
        ...
        check(
            "and a newly pooled conn is handed out on the next call",
            e3.IsNil() && got3.is_some(),
            fmt::Sprintf!("%v", e3),
        );
    }
    ...
    fmt::Printf!("\n%d passed, %d failed\n", p as i64, f as i64);
    if f == 0 {
        fmt::Printf!("HTTP_IDLEPOOL_SMOKE_OK\n");
        goish::os::Exit(0);
    }
