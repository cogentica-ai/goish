...
fn main() {
    ...
    // 1. MkdirAll creates a 3-level deep tree.
    {
        ...
        if err.IsNil() && serr.IsNil() && fi.IsDir() {
            fmt::Println!("[ 1] MkdirAll deep             PASS");
        } else {
            fmt::Println!("[ 1] MkdirAll deep             FAIL");
            failed += 1;
        }
    }
    ...
    // 2. MkdirAll on existing dir returns nil (idempotent).
    {
        ...
        if err.IsNil() {
            fmt::Println!("[ 2] MkdirAll idempotent       PASS");
        } else {
            fmt::Println!("[ 2] MkdirAll idempotent       FAIL");
            failed += 1;
        }
    }
    ...
    // 3. Files inside the tree.
    {
        ...
        if err.IsNil() && data.Len() == 3 {
            fmt::Println!("[ 3] write into deep dir       PASS");
        } else {
            fmt::Println!("[ 3] write into deep dir       FAIL");
            failed += 1;
        }
    }
    ...
    // 4. RemoveAll removes the entire tree, files included.
    {
        ...
        if err.IsNil() && !serr.IsNil() {
            fmt::Println!("[ 4] RemoveAll cleans tree     PASS");
        } else {
            fmt::Println!("[ 4] RemoveAll cleans tree     FAIL err={}", err.IsNil());
            failed += 1;
        }
    }
    ...
    // 5. RemoveAll on a missing path returns nil.
    {
        ...
        if err.IsNil() {
            fmt::Println!("[ 5] RemoveAll missing → nil   PASS");
        } else {
            fmt::Println!("[ 5] RemoveAll missing → nil   FAIL");
            failed += 1;
        }
    }
