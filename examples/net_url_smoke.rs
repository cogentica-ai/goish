...
fn main() {
    ...
    check(
        u.Scheme == string::from_static("https"),
        b"url: Scheme wrong\n",
    );
    check(
        u.Host == string::from_static("example.com:8080"),
        b"url: Host wrong\n",
    );
    check(
        u.Path == string::from_static("/the/path"),
        b"url: Path wrong\n",
    );
    check(
        u.RawQuery == string::from_static("q=1&r=2"),
        b"url: RawQuery wrong\n",
    );
    check(
        u.Fragment == string::from_static("frag"),
        b"url: Fragment wrong\n",
    );
    check(
        u.Hostname() == string::from_static("example.com"),
        b"url: Hostname() wrong\n",
    );
    check(
        u.Port() == string::from_static("8080"),
        b"url: Port() wrong\n",
    );
    ...
    check(
        uname == string::from_static("alice"),
        b"url: Username wrong\n",
    );
    let (pwd, ok) = u.User.Password();
    check(ok, b"url: Password ok wrong\n");
    check(
        pwd == string::from_static("secret"),
        b"url: Password wrong\n",
    );
    ...
    check(
        r.Path == string::from_static("/just/a/path"),
        b"url: relative Path wrong\n",
    );
    ...
    check(
        esc == string::from_static("hello+world+%26+more"),
        b"url: QueryEscape wrong\n",
    );

    let (unesc, err) = url::QueryUnescape("hello+world+%26+more");
    check(err == nil, b"url: QueryUnescape error\n");
    check(
        unesc == string::from_static("hello world & more"),
        b"url: QueryUnescape wrong\n",
    );
    ...
    // space → %20, / → %2F
    check(
        pesc == string::from_static("a%20b%2Fc"),
        b"url: PathEscape wrong\n",
    );

    let (punesc, err) = url::PathUnescape("a%20b%2Fc");
    check(err == nil, b"url: PathUnescape error\n");
    check(
        punesc == string::from_static("a b/c"),
        b"url: PathUnescape wrong\n",
    );
    ...
    check(
        v == string::from_static("val"),
        b"url: ValuesGet key wrong\n",
    );

    let v2 = url::ValuesGet(&vals, string::from_static("foo"));
    check(
        v2 == string::from_static("bar"),
        b"url: ValuesGet foo first wrong\n",
    );

    check(
        url::ValuesHas(&vals, string::from_static("key")),
        b"url: ValuesHas key wrong\n",
    );
    check(
        !url::ValuesHas(&vals, string::from_static("missing")),
        b"url: ValuesHas missing wrong\n",
    );
