...
fn main() {
    ...
    check(
        !unicode::Is(unicode::Zs, 0x0060),
        b"t2: stride excludes 0x60\n",
    );
    ...
    check(
        unicode::Is(unicode::Zs, 0x3000),
        b"t2: IDEOGRAPHIC SPACE in Zs\n",
    );
    check(!unicode::Is(unicode::Zs, 0x200B), b"t2: ZWSP not Zs\n");

    // ─── 3. Mn (Mark, nonspacing) — 16-bit binary search + 32-bit ──
    check(
        unicode::Is(unicode::Mn, 0x0301),
        b"t3: combining acute in Mn\n",
    );
    ...
    check(
        !unicode::Is(unicode::Mn, 0x05C0),
        b"t3: stride excludes 0x05C0\n",
    );
    ...
    check(
        unicode::Is(unicode::Mn, 0x20D0),
        b"t3: combining harpoon in Mn\n",
    );
    check(
        unicode::Is(unicode::Mn, 0xFE00),
        b"t3: variation selector in Mn\n",
    );
    // 32-bit ranges.
    check(unicode::Is(unicode::Mn, 0xE0100), b"t3: VS17 in Mn (R32)\n");
    check(
        unicode::Is(unicode::Mn, 0x1E94A),
        b"t3: ADLAM nukta in Mn (R32)\n",
    );
    check(!unicode::Is(unicode::Mn, 0x1F600), b"t3: emoji not Mn\n");

    // ─── 4. custom tables (the scanner pattern) + In ───────────────
    check(
        unicode::Is(&ODD_LATIN_AND_MUSIC, 'A' as rune),
        b"t4: A in custom\n",
    );
    check(
        !unicode::Is(&ODD_LATIN_AND_MUSIC, 'B' as rune),
        b"t4: stride excludes B\n",
    );
    check(
        unicode::Is(&ODD_LATIN_AND_MUSIC, 'E' as rune),
        b"t4: E in custom\n",
    );
    check(
        unicode::Is(&ODD_LATIN_AND_MUSIC, 0x00E9),
        b"t4: e-acute in custom\n",
    );
    check(
        unicode::Is(&ODD_LATIN_AND_MUSIC, 0x1D11E),
        b"t4: G clef in custom R32\n",
    );
    check(
        !unicode::Is(&ODD_LATIN_AND_MUSIC, 0x1D200),
        b"t4: past R32 range\n",
    );
    ...
    check(
        !unicode::Is(unicode::Zs, -1),
        b"t5: negative rune not in table\n",
    );
