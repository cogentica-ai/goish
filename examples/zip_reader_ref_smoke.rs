...
// go: none — read a whole archive back through File.Open, the way a
// caller does. The archives come from GO's writer, so every row is a
// real interop check rather than a round trip through goish alone.
fn openrow(idx: int, name: &'static str, data: &'static str, want: &'static str) {
    let raw = unhex(data);
    let shared = alloc::sync::Arc::new(goish::sync::Mutex::new(goish::bytes::NewReader(
        goish::slice::__from_vec(raw.clone()),
    )));
    let (z, err) = zr::NewReader(shared, raw.len() as i64);
    ...
    let mut z = z.unwrap();
    let files = z.File.clone();
    let mut out = string::from_static("");
    for f in files.iter() {
        let (fr, e2) = z.Open(f);
        if e2 != goish::errors::nil {
            out = out
                + fmt::Sprintf!(
                    ";%s,%v,,%s",
                    goish::encoding::hex::EncodeToString(f.FileHeader.Name.as_bytes()),
        ...
        let mut fr = fr.unwrap();
        let mut body: alloc::vec::Vec<u8> = alloc::vec::Vec::new();
        let mut es = string::from_static("");
        loop {
            let mut chunk: goish::slice<goish::byte> =
                goish::slice::__from_vec(alloc::vec![0u8; 64]);
            let (n, e) = fr.Read(&mut chunk);
            if n > 0 {
                body.extend_from_slice(&chunk.as_ref()[..n as usize]);
            }
            if e != goish::errors::nil {
                if !goish::errors::Is(e.clone(), goish::io::EOF) {
                    es = e.Error();
                }
                break;
            }
        ...
        let ce = fr.Close();
        let ces = if ce == goish::errors::nil {
            string::from_static("")
        } else {
            ce.Error()
        };
        out = out
            + fmt::Sprintf!(
                ...
                goish::encoding::hex::EncodeToString(f.FileHeader.Name.as_bytes()),
                f.FileHeader.Method,
                goish::encoding::hex::EncodeToString(&body),
                es,
                ces
            );
    }
...
// go: none — build an archive with goish's Writer and compare it BYTE
// FOR BYTE with the one Go's Writer produced from the same entries.
// This is the other half of the interop check: e647a7c's rows read
// Go's archives, and these write ones Go would recognise.
fn archrow(
    idx: int,
    name: &'static str,
    comment: &'static str,
    ...
    let sink = alloc::sync::Arc::new(goish::sync::Mutex::new(goish::bytes::Buffer::new()));
    let mut w = zw::NewWriter(sink.clone());
    let mut es = string::from_static("");
    if comment.len() > 0 {
        let e = w.SetComment(comment);
        if e != goish::errors::nil {
            es = e.Error();
        }
    }
    if es.Len() == 0 {
        for (nm, deflate, body) in entries.iter() {
            let mut h = zs::FileHeader::default();
            h.Name = string::from_bytes(nm.as_bytes());
            h.Method = if *deflate { zs::Deflate } else { zs::Store };
            h.Flags = flags;
            h.NonUTF8 = nonutf8;
            h.CreatorVersion = creator;
            if modified_unix != 0 {
                h.Modified = goish::time::Unix(modified_unix, 0).UTC();
            }
            let (fw, e) = w.CreateHeader(&h);
            if e != goish::errors::nil {
                es = e.Error();
                break;
            }
            let mut fw = fw.unwrap();
            let (_, e) = fw.Write(goish::slice::__from_vec(body.as_bytes().to_vec()));
            if e != goish::errors::nil {
                es = e.Error();
                break;
            }
    ...
    let got = goish::encoding::hex::EncodeToString(&sink.Lock().Bytes());
    ck(
        idx,
        name,
        fmt::Sprintf!("%s|%s", got, es),
        string::from_bytes(want.as_bytes()),
    );
}
...
fn main() {
    ...
    openrow(159, "stored", "", "|;612e747874,0,68656c6c6f20776f726c64,,");
    openrow(160, "deflated", "", "|;612e747874,8,6161616161616161616161616161616161616161616161616161616161616161616161,,");
    openrow(161, "mixed", "", "|;732e747874,0,73746f726564206279746573,,;642e747874,8,626262626262626262626262626262626262626262626262626262626262,,");
    openrow(162, "empty-content", "504b03041400080000000000000000000000000000000000000005000000652e747874504b0708000000000000000000000000504b0102140014000800000000000000000000000000000000000000050000000000000000000000000000000000652e747874504b0506000000000100010033000000330000000000", "|;652e747874,0,,,");
    openrow(163, "dir", "504b03041400000000000000000000000000000000000000000002000000642f504b0102140014000000000000000000000000000000000000000000020000000000000000000000000000000000642f504b0506000000000100010030000000200000000000", "|;642f,0,,,");
    ...
    archrow(183, "empty", "", &[], 0, false, 0, 0, "504b0506000000000000000000000000000000000000|");
    archrow(184, "one-store", "", &[("a.txt", false, "hello world")], 0, false, 0, 0, "|");
    archrow(185, "two-store", "", &[("a", false, "one"), ("b", false, "two")], 0, false, 0, 0, "|");
    archrow(186, "empty-content", "", &[("e", false, "")], 0, false, 0, 0, "504b0304140008000000000000000000000000000000000000000100000065504b0708000000000000000000000000504b010214001400080000000000000000000000000000000000000001000000000000000000000000000000000065504b050600000000010001002f0000002f0000000000|");
    archrow(187, "dir", "", &[("d/", false, "")], 0, false, 0, 0, "504b03041400000000000000000000000000000000000000000002000000642f504b0102140014000000000000000000000000000000000000000000020000000000000000000000000000000000642f504b0506000000000100010030000000200000000000|");
    archrow(188, "dir-and-file", "", &[("d/", false, ""), ("d/f", false, "x")], 0, false, 0, 0, "|");
    archrow(189, "comment", "the comment", &[("a", false, "z")], 0, false, 0, 0, "|");
    archrow(190, "utf8-name", "", &[("caf\u{e9}.txt", false, "z")], 0, false, 0, 0, "|");
    archrow(191, "one-deflate", "", &[("a.txt", true, "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")], 0, false, 0, 0, "|");
    archrow(192, "mixed", "", &[("s", false, "plain"), ("d", true, "bbbbbbbbbbbbbbbbbbbb")], 0, false, 0, 0, "|");
    archrow(193, "dir-flag8", "", &[("d/", false, "")], 0x8, false, 0, 0, "504b03041400000000000000000000000000000000000000000002000000642f504b0102140014000000000000000000000000000000000000000000020000000000000000000000000000000000642f504b0506000000000100010030000000200000000000|");
    archrow(194, "nonutf8", "", &[("caf\u{e9}", false, "z")], 0, true, 0, 0, "504b03041400080000000000000000000000000000000000000005000000636166c3a97a504b0708af77d2620100000001000000504b0102140014000800000000000000af77d2620100000001000000050000000000000000000000000000000000636166c3a9504b0506000000000100010033000000340000000000|");
    archrow(195, "modified", "", &[("m", false, "z")], 0, false, 1592224244, 0, "|");
    archrow(196, "creator-byte", "", &[("c", false, "z")], 0, false, 0, 0x0314, "504b03041400080000000000000000000000000000000000000001000000637a504b0708af77d2620100000001000000504b0102140314000800000000000000af77d262010000000100000001000000000000000000000000000000000063504b050600000000010001002f000000300000000000|");

    unsafe {
        let (pass, fail) = (PASS, FAIL);
        if pass + fail != 197 {
            fmt::Printf!("FAIL ran %v rows, expected 197\n", pass + fail);
            FAIL += 1;
        }
        let fail = FAIL;
