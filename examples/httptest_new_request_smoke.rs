...
fn main() {
    ...
    // 1. Empty method defaults to GET.
    {
        ...
        if r.Method == "GET" && r.URL.Path == "/foo" {
            fmt::Println!("[ 1] Empty method → GET        PASS");
        } else {
            fmt::Println!("[ 1] Empty method → GET        FAIL method=", r.Method);
            failed += 1;
        }
    }
    ...
    // 2. Explicit method preserved.
    {
        ...
        if r.Method == "POST" && r.URL.Path == "/bar" {
            fmt::Println!("[ 2] POST method               PASS");
        } else {
            fmt::Println!("[ 2] POST method               FAIL");
            failed += 1;
        }
    }
    ...
    // 3. Default Host = "example.com".
    {
        ...
        if r.Host == "example.com" {
            fmt::Println!("[ 3] Host default              PASS");
        } else {
            fmt::Println!("[ 3] Host default              FAIL host=", r.Host);
            failed += 1;
        }
    }
    ...
    // 4. Absolute URL → Host taken from URL.
    {
        ...
        if r.Host == "other.test" {
            fmt::Println!("[ 4] Host from absolute URL    PASS");
        } else {
            fmt::Println!("[ 4] Host from absolute URL    FAIL host=", r.Host);
            failed += 1;
        }
    }
    ...
    // 5. Proto = "HTTP/1.1".
    {
        ...
        if r.Proto == "HTTP/1.1" && r.ProtoMajor == 1 && r.ProtoMinor == 1 {
            fmt::Println!("[ 5] Proto HTTP/1.1            PASS");
        } else {
            fmt::Println!("[ 5] Proto HTTP/1.1            FAIL");
            failed += 1;
        }
    }
    ...
    // 6. RemoteAddr is RFC 5737 TEST-NET.
    {
        ...
        if r.RemoteAddr == "192.0.2.1:1234" {
            fmt::Println!("[ 6] RemoteAddr TEST-NET       PASS");
        } else {
            fmt::Println!("[ 6] RemoteAddr TEST-NET       FAIL got=", r.RemoteAddr);
            failed += 1;
        }
    }
    ...
    // 7. Body length carried in ContentLength.
    {
        ...
        if r.ContentLength == 5 {
            fmt::Println!("[ 7] ContentLength = body.Len  PASS");
        } else {
            fmt::Println!("[ 7] ContentLength = body.Len  FAIL got=", r.ContentLength);
            failed += 1;
        }
    }
    ...
    // 8. NewRequestWithContext also works.
    {
        ...
        if r.Method == "GET" && r.URL.Path == "/ctx" {
            fmt::Println!("[ 8] WithContext               PASS");
        } else {
            fmt::Println!("[ 8] WithContext               FAIL");
            failed += 1;
        }
    }
