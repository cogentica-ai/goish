...
//  11-16. dict_decoder.go's copy paths, on streams produced by a
//      running Go 1.25.5 (tools/gen_flate_dict_ref.go through
//      scripts/goref.sh) and chosen to drive them: a 1000-byte run of
//      one character and a 2-byte repeat both give dist < length, so
//      writeCopy propagates bytes it has just written; a 40 KB
//      period-7 pattern and a 120 KB doubled block cross the 32 KiB
//      window, so writeCopy's wrap branch runs and readFlush resets
//      both cursors; and a 70 KB non-repeating stream forces the
//      stored/short-match mix. The expected output is regenerated
//      here rather than embedded, so all 236 KB of it is compared.
...
fn main() {
    go!(|| {
        ...
        if f == 0 {
            ...
            fmt::Println!("FAIL", f as i64, "of 16");
            syscall::Exit(1);
        }
    });
    schedule();
...
// The six Go-reference inputs, regenerated rather than embedded: the
// largest is 120 KB, and a literal would dwarf the file.
#[derive(Clone, Copy)]
enum Gen {
    Run1000,
    Ab5000,
...
fn gen(kind: Gen, n: usize) -> alloc::vec::Vec<u8> {
    let mut v: alloc::vec::Vec<u8> = alloc::vec::Vec::with_capacity(n);
    let mut i: usize = 0;
    while i < n {
        let b = match kind {
            Gen::Run1000 => b'a',
            Gen::Ab5000 => b"ab"[i % 2],
            Gen::Abc3 => b"abc"[i % 3],
            Gen::Period7 => b'a' + (i % 7) as u8,
            Gen::Window => b"xyz"[(i % 60000) % 3],
            Gen::Mixed => (((i * 7) % 251) & 0x7f) as u8,
        };
        v.push(b);
        i += 1;
    }
...
fn check_gen(idx: u8, kind: Gen, n: usize, compressed: &[u8], label: &[u8]) {
    let want = gen(kind, n);
    check(idx, &want, compressed, label);
}
