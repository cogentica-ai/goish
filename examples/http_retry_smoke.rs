...
fn req(method: &'static str, idem: &'static str) -> Request {
    ...
    if !idem.is_empty() { r.Header.Set(string(idem), string("k")); }
    r
}
...
fn run() {
    // ── isReplayable ──
    {
        ...
        let mut bad = string("");
        for m in idempotent {
            if !req(m, "").isReplayable() { bad = fmt::Sprintf!("%s should replay", string(*m)); }
        }
        for m in not {
            if req(m, "").isReplayable() { bad = fmt::Sprintf!("%s should NOT replay", string(*m)); }
        ...
        check("isReplayable: idempotent methods, empty method, and the two opt-in headers",
              bad.Len() == 0, bad);
    }
    ...
    // ── shouldRetryRequest ──
    {
        ...
        check("nothing-written retries even a POST",
              shouldRetryRequest(&post, errNothingWritten.into(), true), string(""));

        // A reused conn retries an idempotent request on these two.
        check("reused + idempotent retries on read-from-server and server-closed-idle",
        ...
        check("a reused conn does NOT retry a POST on a mid-flight failure",
        ...
        check("an unrecognised error does not retry",
              !shouldRetryRequest(&get, other, true), string(""));
    }

    let p = PASSED.load(Ordering::Relaxed);
    let f = FAILED.load(Ordering::Relaxed);
    fmt::Printf!("\n%d passed, %d failed\n", p as i64, f as i64);
    if f == 0 { fmt::Printf!("HTTP_RETRY_SMOKE_OK\n"); goish::os::Exit(0); }
