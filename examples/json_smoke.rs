...
fn main() {
    ...
    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"true", &mut v);
    check(
        err == nil && v.AsBool() == Some(true),
        b"json: true wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"false", &mut v);
    check(
        err == nil && v.AsBool() == Some(false),
        b"json: false wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"42", &mut v);
    check(
        err == nil && v.AsNumber() == Some(42.0),
        b"json: number int wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"-3.14", &mut v);
    check(
        err == nil && v.AsNumber() == Some(-3.14),
        b"json: number neg-float wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"1.5e2", &mut v);
    check(
        err == nil && v.AsNumber() == Some(150.0),
        b"json: number sci wrong\n",
    );

    // ─── Unmarshal — strings (with escapes) ──────────────────────────

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"\"hi\"", &mut v);
    check(
        err == nil && v.AsString().unwrap().clone() == "hi",
        b"json: string wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"\"a\\nb\"", &mut v);
    check(
        err == nil && v.AsString().unwrap().clone() == "a\nb",
        b"json: \\n escape wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"\"q\\\"q\"", &mut v);
    check(
        err == nil && v.AsString().unwrap().clone() == "q\"q",
        b"json: \\\" escape wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"\"\\u0041\"", &mut v);
    check(
        err == nil && v.AsString().unwrap().clone() == "A",
        b"json: \\u escape wrong\n",
    );
    ...
    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"[1, 2, 3]", &mut v);
    ...
    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"[\"a\", true, null]", &mut v);
    check(err == nil, b"json: mixed array err\n");
    let arr = v.AsArray().unwrap();
    check(arr.Len() == 3, b"json: mixed array len\n");
    check(
        arr[0].AsString().unwrap().clone() == "a",
        b"json: mixed[0] wrong\n",
    );
    ...
    let err = json::Unmarshal(b"{}", &mut v);
    check(
        err == nil && v.AsObject().unwrap().Len() == 0,
        b"json: empty obj wrong\n",
    );

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"{\"name\":\"alice\",\"count\":3}", &mut v);
    check(err == nil, b"json: obj err\n");
    let obj = v.AsObject().unwrap();
    check(obj.Len() == 2, b"json: obj len wrong\n");
    let (name, ok) = obj.Get(string("name"));
    check(
        ok && name.AsString().unwrap().clone() == "alice",
        b"json: obj[name] wrong\n",
    );
    let (count, ok) = obj.Get(string("count"));
    check(
        ok && count.AsNumber() == Some(3.0),
        b"json: obj[count] wrong\n",
    );
    ...
    let mut v = json::Value::Null;
    let err = json::Unmarshal(nested, &mut v);
    ...
    check(
        inner_b.AsString().unwrap().clone() == "c",
        b"json: nested deep wrong\n",
    );

    // ─── Unmarshal — whitespace tolerance ────────────────────────────

    let mut v = json::Value::Null;
    let err = json::Unmarshal(b"  {  \"k\"  :  42  }  ", &mut v);
    check(err == nil, b"json: whitespace err\n");
    let obj = v.AsObject().unwrap();
    let (k, _) = obj.Get(string("k"));
    check(
        k.AsNumber() == Some(42.0),
        b"json: whitespace value wrong\n",
    );

    // ─── Unmarshal — errors ──────────────────────────────────────────

    let mut _v = json::Value::Null;
    let err = json::Unmarshal(b"{", &mut _v);
    check(err != nil, b"json: truncated obj must err\n");

    let mut _v = json::Value::Null;
    let err = json::Unmarshal(b"abc", &mut _v);
    check(err != nil, b"json: garbage must err\n");

    let mut _v = json::Value::Null;
    let err = json::Unmarshal(b"[1,]", &mut _v);
    ...
    check(
        string(b) == "\"a\\nb\\\"c\"",
        b"json: marshal str escape wrong\n",
    );
    ...
    check(
        string(b) == "{\"a\":false,\"z\":1}",
        b"json: marshal sorted-object wrong\n",
    );
    ...
    for input in inputs {
        ...
        let mut v2 = json::Value::Null;
        let err = json::Unmarshal(&out, &mut v2);
