...
#[goish::main]
fn main() {
    let c: [chan<i64>; 3] = [make!(chan i64), make!(chan i64, 2), make!(chan i64, 3)];
    ...
    check(
        ...
        b"chan_micro_select_3chan: not all done\n",
    );
    check(
        SEND_TOTAL.load(Ordering::Relaxed) == 3 * N,
        b"chan_micro_select_3chan: send total wrong\n",
    );
    check(
        RECV_TOTAL.load(Ordering::Relaxed) == 3 * N,
        b"chan_micro_select_3chan: recv total wrong\n",
    );
