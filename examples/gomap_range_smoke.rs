...
fn main() {
    ...
    struct Holder { inner: goish::map<string, int> }
    ...
    m2["a"] = 1; m2["b"] = 2; m2["c"] = 3; m2["d"] = 4; m2["e"] = 5;
    check(maps::Equal(&m, &m2), b"maps::Equal: identical maps not equal\n");

    m2["a"] = 99;
    check(!maps::Equal(&m, &m2), b"maps::Equal: different maps reported equal\n");

    // ── maps::Clone ───────────────────────────────────────────────────
    let c = maps::Clone(&m);
    check(maps::Equal(&m, &c), b"maps::Clone: clone not equal to original\n");

    // Mutating the clone must not affect the original.
    let mut c2 = maps::Clone(&m);
    c2["a"] = 999;
    check(m["a"] == 1, b"maps::Clone: mutating clone changed original\n");
    ...
    check(!fuzzy_ne, b"maps::EqualFunc: fuzzy unequal reported equal\n");
    ...
    check(df.Has(string::from_static("keep1")),        b"maps::DeleteFunc: keep1 missing\n");
    check(df.Has(string::from_static("keep2")),        b"maps::DeleteFunc: keep2 missing\n");
    check(!df.Has(string::from_static("drop1")),       b"maps::DeleteFunc: drop1 still present\n");
    check(!df.Has(string::from_static("drop2")),       b"maps::DeleteFunc: drop2 still present\n");

    // ── Range over int-keyed map (different key type) ─────────────────
    let mut im = goish::make!(map[int]int);
    let n: int = 50;
    let mut i: int = 0;
    while i < n { im[i] = i * 2; i += 1; }
