...
// tls_server_smoke uses, for the client<->server loopback below.
...
fn main() {
    ...
    // ── client<->server loopback: the verbatim drivers, end to end ──
    //
    // The strongest test of Conn.clientHandshake, Conn.serverHandshake,
    // and both {client,server}HandshakeState.handshake drivers: two
    // verbatim conns over an in-memory pipe, each on its own goroutine.
    // The random ephemerals and signatures cancel out — a completed
    // handshake with matching traffic secrets on both halves proves the
    // whole path, for TLS 1.3 and TLS 1.2. Run inside a goroutine (the
    // handshake blocks on the pipe) and drive the scheduler from main.
    {
        let (cert, kperr) = tls::X509KeyPair(CERT_PEM, KEY_PEM);
        check("the loopback cert+key parse", kperr.IsNil());
        ...
        // completion is itself the proof: both Finished MACs are taken
        // over the master secret, so mutual success means the master
        // secrets agreed.
        let (c12e, s12e, _m12, cv12, _) = tls::handshake_loopback(cert, false);
        eq("TLS 1.2 loopback: the client handshake succeeds", c12e, "");
        eq(
            "TLS 1.2 loopback: the server handshake succeeds (Finished MACs agree)",
            s12e,
            "",
        );
