...
fn parse(raw: &'static str) -> string {
    let mut br = bufio::NewReader(bytes::NewReader(
        goish::slice::<u8>::__from_vec(raw.as_bytes().to_vec()),
    ));
