...
use goish::fmt;
use goish::net::http::transport::{connectMethod, persistConn, wantConn, wantConnQueue, Waiter};
...
fn run() {
    ...
    // A fresh waiter is waiting; delivery ends that, once.
    {
        ...
        check(
            "delivery succeeds once and only once, and ends the wait",
            first && !second && !w.waiting(),
            string(""),
        );
    }
    ...
    // An error delivery is equally valid and equally final.
    {
        ...
        check(
            "an error can be delivered instead of a conn",
            ok && !w.waiting() && w.__delivered().is_none(),
            string(""),
        );
    }
    ...
    // Cancelling AFTER delivery returns the conn to the pool.
    {
        ...
        check(
            "cancel after delivery hands the conn back to the pool",
            pc.isReused() && !pc.isBroken(),
            string(""),
        );
        ...
        check(
            "the handed-back conn occupies a real pool slot",
            e1.IsNil() && !e2.IsNil(),
    ...
    // Cancelling BEFORE delivery just ends the wait.
    {
        ...
        w.cancel(&t);
        check(
            "cancel before delivery ends the wait with nothing to return",
            !w.waiting() && w.__delivered().is_none(),
            string(""),
        );
    }
    ...
    // The queue drops waiters that are no longer waiting.
    {
        ...
        check(
            "cleanFrontNotWaiting drops a delivered waiter from the front",
            cleaned && q.len() == 1,
            fmt::Sprintf!("cleaned=%v len=%d", cleaned, q.len()),
        );
        // b is still waiting, so a second sweep changes nothing.
        check(
            "and stops at the first still-waiting entry",
            !q.cleanFrontNotWaiting() && q.len() == 1,
            string(""),
        );
    }
    ...
    fmt::Printf!("\n%d passed, %d failed\n", p as i64, f as i64);
    if f == 0 {
        fmt::Printf!("HTTP_WANTCONN_SMOKE_OK\n");
        goish::os::Exit(0);
    }
