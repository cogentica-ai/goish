// go: file html/entity.go decls:
//
// entity.go — the HTML5 named character references.
//
// GENERATED from the Go 1.25.5 source by the snippet recorded in
// tools/gen_html_ref.go's commit message, not transcribed. Two facts
// about this table drive the lookup in escape.rs and are easy to miss:
//
//   * Some names have NO trailing semicolon ("&amp" as well as
//     "&amp;"), which is why an unterminated reference still decodes
//     and why `&notreal;` becomes "\u{ac}real;" — `&not` matches.
//   * The names are CASE SENSITIVE per entry: "&AMP;" and "&amp;" are
//     both in the table and "&Amp;" is not.
//
// The table is sorted by name so it can be binary-searched.

#![allow(non_snake_case)]

use crate::types::rune;

// go: sdk 1.25.5 html/entity.go:10-10 longestEntityWithoutSemicolon
/// Go: "longestEntityWithoutSemicolon is the length of the longest
/// entity name without a trailing semicolon."
pub(crate) const longestEntityWithoutSemicolon: usize = 6;

// go: sdk 1.25.5 html/entity.go:21-2261 entityMaps
/// Go builds BOTH maps inside one `var entityMaps = sync.OnceValues(…)`
/// closure, so the whole block is a single declaration and both of
/// goish's tables anchor to it. goish has them as two plain statics:
/// there is nothing to build lazily when the table is already data./// Go: `var entity = map[string]rune{…}` — 2231 names, sorted here.
pub(crate) static entity: [(&str, rune); 2138] = [
    ("AElig", 0x00C6),
    ("AElig;", 0x00C6),
    ("AMP", 0x0026),
    ("AMP;", 0x0026),
...
// go: sdk 1.25.5 html/entity.go:21-2261 entityMaps
/// Go: `var entity2 = map[string][2]rune{…}` — the names that
/// expand to TWO runes.
pub(crate) static entity2: [(&str, rune, rune); 93] = [
