...
fn err_base_len() -> error      { ERR_BASE_LEN.into() }
fn err_calc_len() -> error      { ERR_CALC_LEN.into() }
fn err_reserved() -> error      { ERR_RESERVED.into() }
fn err_too_many_ptr() -> error  { ERR_TOO_MANY_PTR.into() }
fn err_invalid_ptr() -> error   { ERR_INVALID_PTR.into() }
fn err_invalid_name() -> error  { ERR_INVALID_NAME.into() }
fn err_resource_len() -> error  { ERR_RESOURCE_LEN.into() }
fn err_seg_too_long() -> error  { ERR_SEG_TOO_LONG.into() }
fn err_name_too_long() -> error { ERR_NAME_TOO_LONG.into() }
fn err_zero_seg_len() -> error  { ERR_ZERO_SEG_LEN.into() }
fn err_res_too_long() -> error  { ERR_RES_TOO_LONG.into() }
fn err_non_canonical_name() -> error { ERR_NON_CANONICAL_NAME.into() }
fn err_string_too_long() -> error    { ERR_STRING_TOO_LONG.into() }
fn err_param_out_of_order() -> error { ERR_PARAM_OUT_OF_ORDER.into() }
fn err_too_long_svcb_value() -> error { ERR_TOO_LONG_SVCB_VALUE.into() }
...
impl Header {
    fn pack(&self) -> (u16, u16) {
        ...
        if self.RecursionAvailable { bits |= HEADER_BIT_RA; }
        if self.AuthenticData      { bits |= HEADER_BIT_AD; }
        if self.CheckingDisabled   { bits |= HEADER_BIT_CD; }
        if self.RecursionDesired   { bits |= HEADER_BIT_RD; }
        if self.Truncated          { bits |= HEADER_BIT_TC; }
        if self.Authoritative      { bits |= HEADER_BIT_AA; }
        if self.Response           { bits |= HEADER_BIT_QR; }
        (id, bits)
    }
}
...
impl Default for Section { fn default() -> Self { Section::NotStarted } }
...
impl Default for Name {
    fn default() -> Self { Name { Data: [0u8; 255], Length: 0 } }
}
...
impl Name {
    ...
    fn pack(
        ...
        while i <= self.Length as usize {
            if i == self.Length as usize || name_bytes[i] == b'.' {
                ...
                if label_len >= 64 { return Err(err_seg_too_long()); }
                if label_len == 0  { return Err(err_zero_seg_len()); }
    ...
    fn unpack(&mut self, msg: &[u8], off: usize) -> Result<usize, error> {
        ...
        loop {
            ...
            match c & 0xC0 {
                0x00 => {
                    ...
                    if end_off > msg.len() { return Err(err_calc_len()); }
                    for &b in &msg[curr_off..end_off] {
                        if b == b'.' { return Err(err_invalid_name()); }
                    }
                ...
                0xC0 => {
                    if curr_off >= msg.len() { return Err(err_invalid_ptr()); }
                    let c1 = msg[curr_off];
                    curr_off += 1;
                    if ptr_count == 0 { new_off = curr_off; }
                    ptr_count += 1;
                    if ptr_count > 10 { return Err(err_too_many_ptr()); }
        ...
        if name.len() > NON_ENCODED_NAME_MAX { return Err(err_name_too_long()); }
...
fn skip_name(msg: &[u8], off: usize) -> Result<usize, error> {
    let mut new_off = off;
    loop {
        if new_off >= msg.len() { return Err(err_base_len()); }
        ...
        match c & 0xC0 {
            0x00 => {
                if c == 0x00 { break; }
                new_off += c;
                if new_off > msg.len() { return Err(err_calc_len()); }
            }
            0xC0 => { new_off += 1; break; }
            _ => return Err(err_reserved()),
        }
    }
    Ok(new_off)
...
pub fn MustNewName<S: AsRef<str>>(name: S) -> Name {
    ...
    if err != crate::errors::nil { panic!("creating name"); }
    n
}
...
impl ResourceHeader {
    pub fn SetEDNS0(&mut self, udp_payload_len: usize, ext_rcode: RCode, dnssec_ok: bool) -> error {
        ...
        if dnssec_ok { self.TTL |= EDNS0_DNSSEC_OK; }
        crate::errors::nil
    }
    ...
    fn unpack(&mut self, msg: &[u8], off: usize) -> Result<usize, error> {
        let mut o = off;
        o = self.Name.unpack(msg, o).map_err(|e| nested_err("Name", e))?;
...
fn skip_resource_wire(msg: &[u8], off: usize) -> Result<usize, error> {
    ...
    if off > msg.len() { return Err(err_resource_len()); }
    Ok(off)
}
...
#[derive(Clone, Default, Debug)]
pub struct AResource    { pub A:    [u8; 4]  }

#[derive(Clone, Default, Debug)]
pub struct AAAAResource { pub AAAA: [u8; 16] }

#[derive(Clone, Default, Debug)]
pub struct CNAMEResource { pub CNAME: Name }

#[derive(Clone, Default, Debug)]
pub struct NSResource    { pub NS: Name }

#[derive(Clone, Default, Debug)]
pub struct PTRResource   { pub PTR: Name }

#[derive(Clone, Default, Debug)]
pub struct MXResource    { pub Pref: u16, pub MX: Name }

#[derive(Clone, Default, Debug)]
pub struct SOAResource {
    pub NS: Name, pub MBox: Name,
    pub Serial: u32, pub Refresh: u32, pub Retry: u32,
    pub Expire: u32, pub MinTTL: u32,
}

#[derive(Clone, Default, Debug)]
pub struct TXTResource { pub TXT: Vec<String> }

#[derive(Clone, Default, Debug)]
pub struct SRVResource {
    pub Priority: u16, pub Weight: u16,
    pub Port: u16, pub Target: Name,
}

#[derive(Clone, Default, Debug)]
pub struct OPTResource { pub Options: Vec<DNSOption> }

#[derive(Clone, Default, Debug)]
pub struct DNSOption { pub Code: u16, pub Data: Vec<u8> }

#[derive(Clone, Default, Debug)]
pub struct UnknownResource { pub Type: Type, pub Data: Vec<u8> }
...
#[derive(Clone, Default, Debug)]
pub struct HTTPSResource { pub SVCBResource: SVCBResource }

#[derive(Clone, Default, Debug)]
pub struct SVCParam { pub Key: SVCParamKey, pub Value: Vec<u8> }
...
impl ResourceBody for AResource {
    fn pack_body(&self, mut msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, _co: usize) -> Result<Vec<u8>, error> {
        msg.extend_from_slice(&self.A); Ok(msg)
    }
    fn real_type(&self) -> Type { TypeA }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for AAAAResource {
    fn pack_body(&self, mut msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, _co: usize) -> Result<Vec<u8>, error> {
        msg.extend_from_slice(&self.AAAA); Ok(msg)
    }
    fn real_type(&self) -> Type { TypeAAAA }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for CNAMEResource {
    fn pack_body(&self, msg: Vec<u8>, c: Option<&mut BTreeMap<String,u16>>, co: usize) -> Result<Vec<u8>, error> {
        self.CNAME.pack(msg, c, co)
    }
    fn real_type(&self) -> Type { TypeCNAME }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}
...
impl ResourceBody for PTRResource {
    ...
    fn real_type(&self) -> Type { TypePTR }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for MXResource {
    fn pack_body(&self, mut msg: Vec<u8>, c: Option<&mut BTreeMap<String,u16>>, co: usize) -> Result<Vec<u8>, error> {
        msg = pack_u16(msg, self.Pref);
        self.MX.pack(msg, c, co).map_err(|e| nested_err("MXResource.MX", e))
    }
    fn real_type(&self) -> Type { TypeMX }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for SOAResource {
    fn pack_body(&self, msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, co: usize) -> Result<Vec<u8>, error> {
        let msg = self.NS.pack(msg, None, co).map_err(|e| nested_err("SOAResource.NS", e))?;
        let msg = self.MBox.pack(msg, None, co).map_err(|e| nested_err("SOAResource.MBox", e))?;
    ...
    fn real_type(&self) -> Type { TypeSOA }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for TXTResource {
    fn pack_body(&self, mut msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, _co: usize) -> Result<Vec<u8>, error> {
        for s in &self.TXT { msg = pack_text(msg, s)?; }
        Ok(msg)
    }
    fn real_type(&self) -> Type { TypeTXT }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for SRVResource {
    fn pack_body(&self, mut msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, co: usize) -> Result<Vec<u8>, error> {
        msg = pack_u16(msg, self.Priority);
        msg = pack_u16(msg, self.Weight);
        msg = pack_u16(msg, self.Port);
        self.Target.pack(msg, None, co).map_err(|e| nested_err("SRVResource.Target", e))
    }
    fn real_type(&self) -> Type { TypeSRV }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}
...
impl ResourceBody for OPTResource {
    ...
    fn real_type(&self) -> Type { TypeOPT }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for UnknownResource {
    fn pack_body(&self, mut msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, _co: usize) -> Result<Vec<u8>, error> {
        msg.extend_from_slice(&self.Data); Ok(msg)
    }
    fn real_type(&self) -> Type { self.Type }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}

impl ResourceBody for SVCBResource {
    fn pack_body(&self, msg: Vec<u8>, _c: Option<&mut BTreeMap<String,u16>>, _co: usize) -> Result<Vec<u8>, error> {
        let mut msg = pack_u16(msg, self.Priority);
        msg = self.Target.pack(msg, None, 0).map_err(|e| nested_err("SVCBResource.Target", e))?;
        let mut prev_key: Option<u16> = None;
        for param in &self.Params {
            if let Some(pk) = prev_key {
                if param.Key <= pk { return Err(nested_err("SVCBResource.Params", err_param_out_of_order())); }
            }
            if param.Value.len() > 65535 { return Err(nested_err("SVCBResource.Params", err_too_long_svcb_value())); }
    ...
    fn real_type(&self) -> Type { TypeSVCB }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}
...
impl ResourceBody for HTTPSResource {
    ...
    fn real_type(&self) -> Type { TypeHTTPS }
    fn clone_box(&self) -> Box<dyn ResourceBody> { Box::new(self.clone()) }
}
...
fn unpack_a_resource(msg: &[u8], off: usize) -> Result<AResource, error> {
    let mut a = [0u8; 4];
    if off + 4 > msg.len() { return Err(err_base_len()); }
...
fn unpack_aaaa_resource(msg: &[u8], off: usize) -> Result<AAAAResource, error> {
    let mut aaaa = [0u8; 16];
    if off + 16 > msg.len() { return Err(err_base_len()); }
...
fn unpack_soa_resource(msg: &[u8], off: usize) -> Result<SOAResource, error> {
    ...
    Ok(SOAResource { NS: ns, MBox: mbox, Serial: serial, Refresh: refresh,
                     Retry: retry, Expire: expire, MinTTL: min_ttl })
}
...
fn unpack_txt_resource(msg: &[u8], off: usize, length: u16) -> Result<TXTResource, error> {
    ...
    while n < length {
        ...
        if length - n < t.len() as u16 + 1 { return Err(err_calc_len()); }
        n += t.len() as u16 + 1;
        off = new_off;
        txts.push(t);
    }
...
fn unpack_srv_resource(msg: &[u8], off: usize) -> Result<SRVResource, error> {
    ...
    target.unpack(msg, off).map_err(|e| nested_err("Target", e))?;
    Ok(SRVResource { Priority: priority, Weight: weight, Port: port, Target: target })
}
...
fn unpack_opt_resource(msg: &[u8], off: usize, length: u16) -> Result<OPTResource, error> {
    ...
    while off < end {
        ...
        let llen = l as usize;
        if no + llen > msg.len() { return Err(nested_err("Data", err_calc_len())); }
        let data = msg[no..no+llen].to_vec();
        off = no + llen;
        opts.push(DNSOption { Code: code, Data: data });
    }
    Ok(OPTResource { Options: opts })
}

fn unpack_unknown_resource(rtype: Type, msg: &[u8], off: usize, length: u16) -> Result<UnknownResource, error> {
    let llen = length as usize;
    if off + llen > msg.len() { return Err(err_base_len()); }
    Ok(UnknownResource { Type: rtype, Data: msg[off..off+llen].to_vec() })
}
...
fn unpack_svcb_resource(msg: &[u8], off: usize, length: u16) -> Result<SVCBResource, error> {
    ...
    r.Priority = priority;
    params_off = r.Target.unpack(msg, params_off).map_err(|e| nested_err("Target", e))?;
    ...
    while tmp_off < body_end {
        ...
        if let Some(pk) = prev_key {
            if key <= pk { return Err(nested_err("Params", err_param_out_of_order())); }
        }
        prev_key = Some(key);
    ...
    if tmp_off != body_end { return Err(err_resource_len()); }
    ...
    for _ in 0..n {
        ...
        r.Params.push(SVCParam { Key: key, Value: msg[no..no+llen].to_vec() });
        off = no + llen;
    }
    Ok(r)
}
...
impl Parser {
    pub fn new() -> Self { Parser::default() }
    ...
    fn check_advance(&mut self, sec: Section) -> error {
        if self.section < sec   { return ErrNotStarted.clone().into(); }
        if self.section > sec   { return ErrSectionDone.clone().into(); }
    ...
    fn resource_header_inner(&mut self, sec: Section) -> (ResourceHeader, error) {
        ...
        if err != crate::errors::nil { return (ResourceHeader::default(), err); }
    ...
    fn skip_resource_inner(&mut self, sec: Section) -> error {
        if self.res_header_valid && self.section == sec {
            ...
            if new_off > self.msg.len() { return err_resource_len(); }
            self.off = new_off;
            self.res_header_valid = false;
            self.index += 1;
        ...
        let err = self.check_advance(sec);
        if err != crate::errors::nil { return err; }
        let msg = self.msg.clone();
        match skip_resource_wire(&msg, self.off) {
            Ok(off) => { self.off = off; self.index += 1; crate::errors::nil }
            Err(e)  => {
    ...
    pub fn Question(&mut self) -> (Question, error) {
        ...
        if err != crate::errors::nil { return (Question::default(), err); }
        let mut name = Name::default();
        let off = match name.unpack(&self.msg, self.off) {
            Ok(o) => o,
            Err(e) => return (Question::default(), nested_err("unpacking Question.Name", e)),
        };
        let (typ, off) = match unpack_u16(&self.msg, off) {
            Ok(v) => v,
            Err(e) => return (Question::default(), nested_err("unpacking Question.Type", e)),
        };
        let (class, off) = match unpack_u16(&self.msg, off) {
            Ok(v) => v,
            Err(e) => return (Question::default(), nested_err("unpacking Question.Class", e)),
        };
        self.off = off;
        self.index += 1;
        (Question { Name: name, Type: typ, Class: class }, crate::errors::nil)
    }
    ...
    pub fn SkipQuestion(&mut self) -> error {
        ...
        let off = match skip_u16(&msg, off) {
            Ok(o) => o, Err(e) => return nested_err("skipping Question Type", e),
        };
        let off = match skip_u16(&msg, off) {
            Ok(o) => o, Err(e) => return nested_err("skipping Question Class", e),
        };
        self.off = off;
        self.index += 1;
        crate::errors::nil
    ...
    pub fn SkipAllQuestions(&mut self) -> error {
        loop {
            ...
            if e == ErrSectionDone { return crate::errors::nil; }
    ...
    pub fn AResource(&mut self) -> (AResource, error) {
        ...
        match unpack_a_resource(&self.msg, self.off) {
            Ok(r) => {
                ...
                self.res_header_valid = false; self.index += 1;
                (r, crate::errors::nil)
            }
    ...
    pub fn SkipAuthority(&mut self) -> error { self.skip_resource_inner(Section::Authorities) }
    ...
    pub fn SkipAdditional(&mut self) -> error { self.skip_resource_inner(Section::Additionals) }
...
pub fn NewBuilder(buf: Vec<u8>, h: Header) -> Builder {
    ...
    Builder {
        ...
        header: WireHeader { id, bits, ..Default::default() },
        start,
        compression: None,
    }
}
...
impl Builder {
    ...
    fn start_check(&self, s: Section) -> error {
        if self.section <= Section::NotStarted { return ErrNotStarted.clone().into(); }
        if self.section > s                    { return ErrSectionDone.clone().into(); }
        crate::errors::nil
    }
    ...
    pub fn Question(&mut self, q: Question) -> error {
        if self.section < Section::Questions { return ErrNotStarted.clone().into(); }
        if self.section > Section::Questions { return ErrSectionDone.clone().into(); }
        let msg = core::mem::take(&mut self.msg);
        match q.pack(msg, self.compression.as_mut(), self.start) {
            Ok(m) => { self.msg = m; self.increment_section_count() }
            Err(e) => e,
        }
    }

    fn check_resource_section(&self) -> error {
        if self.section < Section::Answers     { return ErrNotStarted.clone().into(); }
        if self.section > Section::Additionals { return ErrSectionDone.clone().into(); }
        crate::errors::nil
    }
    ...
    fn add_resource<B: ResourceBody>(&mut self, mut hdr: ResourceHeader, body: &B) -> error {
        ...
        let mut msg = msg;
        if let Err(e) = ResourceHeader::fix_len(&mut msg, len_off, pre_len) { return e; }
        self.msg = msg;
        self.increment_section_count()
    }

    pub fn AResource(&mut self, h: ResourceHeader, r: AResource) -> error     { self.add_resource(h, &r) }
    pub fn AAAAResource(&mut self, h: ResourceHeader, r: AAAAResource) -> error { self.add_resource(h, &r) }
    pub fn CNAMEResource(&mut self, h: ResourceHeader, r: CNAMEResource) -> error { self.add_resource(h, &r) }
    pub fn NSResource(&mut self, h: ResourceHeader, r: NSResource) -> error    { self.add_resource(h, &r) }
    pub fn PTRResource(&mut self, h: ResourceHeader, r: PTRResource) -> error  { self.add_resource(h, &r) }
    pub fn MXResource(&mut self, h: ResourceHeader, r: MXResource) -> error    { self.add_resource(h, &r) }
    pub fn SOAResource(&mut self, h: ResourceHeader, r: SOAResource) -> error  { self.add_resource(h, &r) }
    pub fn TXTResource(&mut self, h: ResourceHeader, r: TXTResource) -> error  { self.add_resource(h, &r) }
    pub fn SRVResource(&mut self, h: ResourceHeader, r: SRVResource) -> error  { self.add_resource(h, &r) }
    pub fn OPTResource(&mut self, h: ResourceHeader, r: OPTResource) -> error  { self.add_resource(h, &r) }
    pub fn UnknownResource(&mut self, h: ResourceHeader, r: UnknownResource) -> error { self.add_resource(h, &r) }
...
fn unpack_u16(msg: &[u8], off: usize) -> Result<(u16, usize), error> {
    if off + 2 > msg.len() { return Err(err_base_len()); }
...
fn unpack_u32(msg: &[u8], off: usize) -> Result<(u32, usize), error> {
    if off + 4 > msg.len() { return Err(err_base_len()); }
    let v = (msg[off] as u32) << 24 | (msg[off+1] as u32) << 16
          | (msg[off+2] as u32) << 8 | msg[off+3] as u32;
    Ok((v, off + 4))
}
...
fn pack_text(mut msg: Vec<u8>, s: &str) -> Result<Vec<u8>, error> {
    if s.len() > 255 { return Err(err_string_too_long()); }
...
fn unpack_text(msg: &[u8], off: usize) -> Result<(String, usize), error> {
    if off >= msg.len() { return Err(err_base_len()); }
    let begin = off + 1;
    let end   = begin + msg[off] as usize;
    if end > msg.len() { return Err(err_calc_len()); }
        ...
        if b < 0x80 {
            ...
            if i + width <= bytes.len() {
                match core::str::from_utf8(&bytes[i..i + width]) {
                    Ok(chunk) => { s.push_str(chunk); i += width; }
                    Err(_)    => { s.push('\u{FFFD}'); i += 1; }
                }
            } else {
                s.push('\u{FFFD}');
                i += 1;
