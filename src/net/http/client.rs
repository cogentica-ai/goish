...
// `__NilRoundTripper` and the `From<Nil> for Box<dyn RoundTripper>`
// impl are auto-generated by `#[goish::interface]` above. Box-shaped
// because `Box<T>` is `#[fundamental]` so the From<Nil> impl lands
// orphan-rule-clean.
//
// `From<Nil> for Arc<dyn RoundTripper + Send + Sync>` — hand-emitted
// here because Arc is non-fundamental, so the macro can't emit this
// impl when the trait is foreign. Inside goish itself (where
// `RoundTripper` is local) the orphan rule does allow it. Users
// writing `pub fn NewTransport(...) -> Arc<dyn RoundTripper> { return
// nil.into(); }` (the goishc-emitted shape for stdlib trait returns)
// rely on this impl.
impl ::core::convert::From<crate::Nil> for alloc::sync::Arc<dyn RoundTripper + Send + Sync> {
    #[inline]
    fn from(_: crate::Nil) -> Self {
        alloc::sync::Arc::new(__NilRoundTripper)
    }
}
