// go: package net/http/internal/testcert
//
// go: file net/http/internal/testcert/testcert.go decls: testingKey, LocalhostCert, LocalhostKey
//
// Go: "Package testcert contains a test-only localhost certificate."
//
// The two PEM blobs are copied byte for byte from Go's file. The key is
// stored with the literal words "TESTING KEY" where "PRIVATE KEY" would
// go, and `testingKey` swaps them back at use — Go does this so that
// credential scanners walking the source tree do not fire on a private
// key that is deliberately public.

#![allow(non_snake_case)]

use crate::gostring::string;

// go: sdk 1.25.5 net/http/internal/testcert/testcert.go:14-34 LocalhostCert
/// Go: "LocalhostCert is a PEM-encoded TLS cert with SAN IPs
/// "127.0.0.1" and "[::1]", expiring at Jan 29 16:00:00 2084 GMT."
pub fn LocalhostCert() -> crate::goslice::slice<crate::types::byte> {
    return crate::convert::bytes(string::from_static(CERT_PEM));
}

// go: sdk 1.25.5 net/http/internal/testcert/testcert.go:36-63 LocalhostKey
/// Go: "LocalhostKey is the private key for LocalhostCert."
pub fn LocalhostKey() -> crate::goslice::slice<crate::types::byte> {
    return crate::convert::bytes(testingKey(string::from_static(KEY_PEM)));
}

// go: sdk 1.25.5 net/http/internal/testcert/testcert.go:65-65 testingKey
fn testingKey(s: string) -> string {
    return crate::strings::ReplaceAll(
        s,
        string::from_static("TESTING KEY"),
        string::from_static("PRIVATE KEY"),
    );
}

// go: none — goish-only: Go writes the PEM inline in the `var`
// initialiser; a Rust `const` needs the literal separated from the
// function that returns it.
const CERT_PEM: &str = r#"-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"#;

// go: none — goish-only, see CERT_PEM.
const KEY_PEM: &str = r#"-----BEGIN RSA TESTING KEY-----

jel6uj2FOP9g54s+GzlSVg/T
-----END RSA TESTING KEY-----"#;
