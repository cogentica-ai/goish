...
// goishlint:ignore GOISH021 ntz8tab, len8tab, rev8tab, m0, m1, m2, m3, m4, m5, deBruijn32, deBruijn64, deBruijn32tab, deBruijn64tab — Go's portable fallbacks: the three lookup tables in bits_tables.go, the six SWAR masks its OnesCount uses, and the two de Bruijn sequences behind TrailingZeros. goish maps each function onto the corresponding Rust `u{N}` method, which is the same LLVM intrinsic Go's compiler emits when it does NOT take the portable path, so there is nothing for a table or a mask to serve. `uintSize` is the unexported half of `UintSize`, which is a const here.
// go: waived errorString.Error — declared in bits_errors_bootstrap.go, which is //go:build compiler_bootstrap — a route the normal Go build does not take and goish does not either. The build that does run uses //go:linkname to reach runtime.overflowError and runtime.divideError, which goish has no linkname for, so Div32/Div64 panic with the same message text instead. port_coverage's other_route check does not know this tag.
// go: waived errorString.RuntimeError — declared in bits_errors_bootstrap.go, which is //go:build compiler_bootstrap — a route the normal Go build does not take and goish does not either. The build that does run uses //go:linkname to reach runtime.overflowError and runtime.divideError, which goish has no linkname for, so Div32/Div64 panic with the same message text instead. port_coverage's other_route check does not know this tag.
//
