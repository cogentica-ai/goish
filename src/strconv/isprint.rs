// go: file strconv/isprint.go decls:
//
// isprint.go — the five range tables IsPrint, IsGraphic and
// isInGraphicList binary-search. Generated in Go by
// `go run makeisprint.go`; transcribed here verbatim, entry for
// entry, because they are data and not code — the goishlint decls
// manifest above is empty for the same reason.
//
// The whole set is 3370 bytes. goish shipped none of it and IsPrint
// answered `true` for every valid non-Latin-1 codepoint instead,
// which made Quote leave unassigned and non-printable runes
// unescaped.

// go: sdk 1.25.5 strconv/isprint.go:11-223 isPrint16
pub(crate) const isPrint16: &[u16] = &[
...
// go: sdk 1.25.5 strconv/isprint.go:226-359 isNotPrint16
pub(crate) const isNotPrint16: &[u16] = &[
...
// go: sdk 1.25.5 strconv/isprint.go:362-616 isPrint32
pub(crate) const isPrint32: &[u32] = &[
...
// go: sdk 1.25.5 strconv/isprint.go:619-731 isNotPrint32
/// Add 0x10000 to each entry.
pub(crate) const isNotPrint32: &[u16] = &[
...
// go: sdk 1.25.5 strconv/isprint.go:735-751 isGraphic
pub(crate) const isGraphic: &[u16] = &[
