// go: file crypto/internal/fips140/bigmod/nat.go decls: not, ctMask, ctEq, bigEndianUint, NewNat, Nat.expand, Nat.reset, Nat.resetToBytes, Nat.trim, Nat.set, Nat.Bits, Nat.Bytes, Nat.SetBytes, Nat.SetOverflowingBytes, Nat.setBytes, Nat.SetUint, Nat.Equal, Nat.IsZero, Nat.IsOne, Nat.IsMinusOne, Nat.IsOdd, Nat.TrailingZeroBitsVarTime, Nat.cmpGeq, Nat.assign, Nat.add, Nat.sub, Nat.ShiftRightVarTime, Nat.BitLenVarTime, Nat.resetFor, Nat.ExpandFor, Nat.shiftIn, Nat.Mod, Nat.maybeSubtractModulus, Nat.Sub, Nat.SubOne, Nat.Add, Nat.montgomeryRepresentation, Nat.montgomeryReduction, Nat.montgomeryMul, Nat.Mul, Nat.Exp, Nat.ExpShortVarTime, Nat.InverseVarTime, Nat.GCDVarTime, Nat.DivShortVarTime, addMulVVW, NewModulus, NewModulusProduct, Modulus.Size, Modulus.BitLen, Modulus.Nat, newModulus, minusInverseModW, rr, bitLen, rshift1, extendedGCD
//
...
//   * `addMulVVW1024/1536/2048` live in nat_noasm.rs, the purego side of
//     Go's build-tag pair — the same choice recorded for nistec.
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:35-35 not
/// `not(c)` (nat.go:35) — logical negation of a `choice`.
fn not(c: choice) -> choice {
    1 ^ c
}

// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:41-41 ctMask
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:45-50 ctEq
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:207-212 bigEndianUint
...
pub(super) const _W_usize: usize = 64; // _W as a usize, for capacity arithmetic
const preallocLimbs: usize = (preallocTarget + _W_usize - 1) / _W_usize;

impl Nat {
    // go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:71-74 NewNat
    ...
    fn montgomeryMul(&mut self, a: &Nat, b: &Nat, m: &Modulus) -> &mut Nat {
        ...
        //
        // Go splits this into a `default` arm and three specialized ones
        // for the RSA sizes, whose comment reads: "The following
        // specialized cases follow the exact same algorithm, but
        // optimized for the sizes most used in RSA. addMulVVW is
        // implemented in assembly with loop unrolling depending on the
        // architecture and bounds checks are removed by the compiler
        // thanks to the constant size."
        //
        // goish keeps the dispatch so the shape matches Go and
        // nat_noasm's three functions have their call sites, but on the
        // purego side every arm reaches the same `addMulVVW` — the
        // specialization is bounds-check and unrolling only, and the
        // results are identical by construction.
        let addMul: fn(&mut [uint], &[uint], uint) -> uint = match n {
            n if n == 1024 / _W_usize => super::nat_noasm::addMulVVW1024,
            n if n == 1536 / _W_usize => super::nat_noasm::addMulVVW1536,
            n if n == 2048 / _W_usize => super::nat_noasm::addMulVVW2048,
            _ => addMulVVW,
        };
        ...
        for i in 0..n {
            ...
            let d = bLimbs[i];
            let c1 = addMul(&mut t[i..n + i], aLimbs, d);
            ...
            let c2 = addMul(&mut t[i..n + i], mLimbs, y);
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:902-916 addMulVVW
...
pub(super) fn addMulVVW(z: &mut [uint], x: &[uint], y: uint) -> uint {
...
impl Modulus {
    // go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:551-554 NewModulus
    ...
    // go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:560-568 NewModulusProduct
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:570-581 newModulus
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:532-544 minusInverseModW
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:481-526 rr
...
// go: none — Go's `>>` on a uint already yields 0 for counts >= _W;
// Rust's panics in debug, so the semantics need spelling out once.
/// Go-semantics right shift of a `uint` word: a shift count `>= _W`
/// yields 0. The count is an `int` to match the `bits::UintSize` index
/// domain in `rr`.
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:452-461 bitLen
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:1200-1212 rshift1
...
// go: sdk 1.25.5 crypto/internal/fips140/bigmod/nat.go:1096-1197 extendedGCD
...
// go: none — Go indexes a []byte directly; goish converts at the
// boundary so no Rust container reaches the public API (AGENTS.md §3).
