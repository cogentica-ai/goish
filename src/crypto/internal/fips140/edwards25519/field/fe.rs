...
impl Element {
    // go: none — Go writes `new(Element)`; this is that constructor.
...
pub(crate) fn rawEqual(&self, u: &Element) -> bool {
    ...
    pub fn Multiply(&mut self, x: &Element, y: &Element) -> &mut Element {
        ...
        let xs = *x;
        let ys = *y;
        super::fe_amd64_noasm::feMul(self, &xs, &ys);
        self
    }
    ...
    pub fn Square(&mut self, x: &Element) -> &mut Element {
        ...
        let xs = *x;
        super::fe_amd64_noasm::feSquare(self, &xs);
        self
    }
...
fn mask64Bits(cond: int) -> u64 {
    ...
    let c = u64::from_ne_bytes((int64(cond)).to_ne_bytes());
    !c.wrapping_sub(1)
}

// go: sdk 1.25.5 crypto/internal/fips140/edwards25519/field/fe.go:332-337 mul51
