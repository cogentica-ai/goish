...
pub(crate) fn blockGeneric(dig: &mut Digest, mut p: &[byte]) {
    ...
    while p.len() >= CHUNK {
        ...
        while i < 64 {
            ...
            let v1 = w[i - 2];
            let t1 = bits::RotateLeft32(v1, -17) ^ bits::RotateLeft32(v1, -19) ^ (v1 >> 10);
            let v2 = w[i - 15];
            let t2 = bits::RotateLeft32(v2, -7) ^ bits::RotateLeft32(v2, -18) ^ (v2 >> 3);
            w[i] = t1
                .wrapping_add(w[i - 7])
                .wrapping_add(t2)
                .wrapping_add(w[i - 16]);
        ...
        while i < 64 {
            ...
            //          + ((e & f) ^ (^e & g)) + _K[i] + w[i]
            let s1 =
                bits::RotateLeft32(e, -6) ^ bits::RotateLeft32(e, -11) ^ bits::RotateLeft32(e, -25);
            let ch = (e & f) ^ ((!e) & g);
            let t1 = h
                .wrapping_add(s1)
                .wrapping_add(ch)
            ...
            //          + ((a & b) ^ (a & c) ^ (b & c))
            let s0 =
                bits::RotateLeft32(a, -2) ^ bits::RotateLeft32(a, -13) ^ bits::RotateLeft32(a, -22);
            let maj = (a & b) ^ (a & c) ^ (b & c);
