// go: file crypto/internal/fips140/rsa/cast.go decls: testPrivateKey, fipsSelfTest
//
// The RSA Cryptographic Algorithm Self-Test (CAST): build the RFC 9500
// §2.1 2048-bit test key, sign a fixed 32-byte digest with SHA-256
// PKCS #1 v1.5, verify it, and compare against the known-answer vector.
//
// Deviations from cast[go] @ Go 1.25.5:
//
//   * Go writes `var fipsSelfTest = sync.OnceFunc(func() { … })`; goish
//     has no `sync.OnceFunc`, so `fipsSelfTest` is a fn guarded by a
//     `SpinLock<bool>` once-flag. Same once-per-process semantics.
//   * Go builds the test key from `bigmod` values directly and sets
//     `fipsApproved: true` by hand (unexported fields are reachable
//     inside the package). goish routes the same components through
//     `NewPrivateKeyWithPrecomputation`, which recomputes
//     `fipsApproved` — for this key that is `true` either way, and the
//     detour buys a validation of the embedded constants.

extern crate alloc;

use super::pkcs1v15::{signPKCS1v15, verifyPKCS1v15};
use super::rsa::{NewPrivateKeyWithPrecomputation, PrivateKey};
use crate::bytes;
use crate::crypto::internal::fips140;
use crate::errors;
use crate::goslice::slice;
use crate::runtime::spin::SpinLock;
use crate::types::byte;

static FIPS_SELF_TEST_DONE: SpinLock<bool> = SpinLock::new(false);

// go: sdk 1.25.5 crypto/internal/fips140/rsa/cast.go:179-234 fipsSelfTest
/// The `sync.OnceFunc`-guarded RSA PKCS #1 v1.5 sign/verify CAST.
pub(super) fn fipsSelfTest() {
    {
        let mut done = FIPS_SELF_TEST_DONE.lock();
        if *done {
            return;
        }
        *done = true;
    }
    fips140::CAST("RSASSA-PKCS-v1.5 2048-bit sign and verify", || {
        let k = testPrivateKey();
        let hash = slice::<byte>::__from_vec(CAST_HASH.to_vec());
        let want = slice::<byte>::__from_vec(CAST_WANT.to_vec());
        let (sig, err) = signPKCS1v15(&k, crate::crypto::SHA256, hash.clone());
        if !err.IsNil() {
            return err;
        }
        let err = verifyPKCS1v15(&k.PublicKey(), crate::crypto::SHA256, hash, sig.clone());
        if !err.IsNil() {
            return err;
        }
        if !bytes::Equal(sig, want) {
            return errors::New("unexpected result");
        }
        return errors::nil;
    });
}

/// The fixed 32-byte digest signed by the CAST (cast.go:182).
static CAST_HASH: [byte; 32] = [
    0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
    0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20,
];

/// The expected PKCS #1 v1.5 SHA-256 signature (cast.go:188).
static CAST_WANT: [byte; 256] = [
    ,
];

// go: sdk 1.25.5 crypto/internal/fips140/rsa/cast.go:16-177 testPrivateKey
/// The RFC 9500 §2.1 2048-bit RSA test private key, used by the CAST.
fn testPrivateKey() -> PrivateKey {
    // https://www.rfc-editor.org/rfc/rfc9500.html#section-2.1
    static N: [byte; 256] = [
        ,
    ];
    static D: [byte; 256] = [
        ,
    ];
    static P: [byte; 128] = [
        ,
    ];
    static Q: [byte; 128] = [
        ,
    ];
    static QINV: [byte; 128] = [
        ,
    ];
    static DP: [byte; 128] = [
        ,
    ];
    static DQ: [byte; 128] = [
        ,
    ];

    let (k, err) = NewPrivateKeyWithPrecomputation(
        slice::<byte>::__from_vec(N.to_vec()),
        65537,
        slice::<byte>::__from_vec(D.to_vec()),
        slice::<byte>::__from_vec(P.to_vec()),
        slice::<byte>::__from_vec(Q.to_vec()),
        slice::<byte>::__from_vec(DP.to_vec()),
        slice::<byte>::__from_vec(DQ.to_vec()),
        slice::<byte>::__from_vec(QINV.to_vec()),
    );
    if !err.IsNil() {
        panic!("crypto/rsa: CAST test key construction failed");
    }
    return k;
}
