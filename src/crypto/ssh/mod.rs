...
//
// go: none — INVENTED. Go's standard library has no crypto/ssh
//     (golang.org/x/crypto/ssh is a separate module), so there is no
//     Go source to anchor against and no tier that can compare this
//     file to anything. It is hand-written crypto, which is the
//     category that produced seven defects across crypto/tls's
//     record.rs and session.rs and net/dnsclient.rs on 2026-09-04.
//
//     Diffed by hand on that date for the classes those turned up.
//     One real finding and one mistake. The mistake: the KEXINIT
//     cookie and the DH private exponent are drawn with their result
//     checked now, but the old `let _ =` was NOT a defect —
//     `crypto::rand`'s reader fatals on failure, like Go's, so it
//     cannot return an error or short-read. The checks are kept as
//     statements of the requirement, not as fixes.
//
//     Still true of this file and worth a reader knowing: the
//     remaining `let _ =` are a hash write, two flow-control window
//     adjustments and a best-effort channel close, none of which
//     carries a secret. Nothing here has been diffed for protocol
//     conformance beyond that, because there is no reference to diff
//     against.
//
// Implements:
...
pub fn Dial<N: Into<string>, A: Into<string>>(
    ...
    // 3. KEXINIT
    // RFC 4253 7.1: the cookie is "a random value generated by the
    // sender". The draw is checked, though the branch is UNREACHABLE:
    // goish's random reader calls `fatal` on failure, as Go's does, so
    // it cannot return an error or a short read. See the note on the
    // DH exponent below.
    let mut client_cookie = [0u8; 16];
    {
        let mut s: slice<byte> = slice::__from_vec(client_cookie.to_vec());
        let mut rng = crate::crypto::rand::RandReader;
        let (n, rerr) = io::Reader::Read(&mut rng, &mut s);
        if !rerr.IsNil() {
            return (make_dead_client(), rerr);
        }
        if n as usize != client_cookie.len() {
            return (
                make_dead_client(),
                ssh_err("ssh: short read from random source"),
            );
        }
        let v = s.__into_vec();
    ...
    // 4. DH KEX
    // The Diffie-Hellman PRIVATE EXPONENT.
    //
    // The draw is checked, and the branch is UNREACHABLE. An earlier
    // version of this comment said an all-zero x would make e = 2^0 =
    // 1 and the shared secret computable by anyone watching. That is
    // true of an all-zero x and irrelevant here: `crypto::rand`'s
    // reader calls `fatal` — which diverges — on any read failure, as
    // Go's does, so the buffer is either filled or the process is
    // gone. The `let _ =` that stood here was correct.
    //
    // The check stays because it costs nothing and states the
