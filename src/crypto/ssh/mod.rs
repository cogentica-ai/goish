...
//     Two found and fixed: the KEXINIT cookie and the Diffie-Hellman
//     private exponent were both drawn with the result discarded, so a
//     failed or short read left them as zeros. The DH one is the
//     serious half — see the comment at the draw.
//
...
pub fn Dial<N: Into<string>, A: Into<string>>(
    ...
    // sender". The result of the draw was discarded here; a failed or
    // short read left it as the zeros it was initialised to.
    ...
    // 4. DH KEX
    // The Diffie-Hellman PRIVATE EXPONENT. The result of this draw was
    // discarded too, and here that is not a weakened secret but no
    // secret at all: an all-zero x makes e = 2^0 mod p = 1, and the
    // shared secret K = f^x mod p = 1 for whatever the server sent.
    // Every key derived from it — both directions' cipher and MAC keys
    // — would then be computable by anyone who watched the handshake,
    // with nothing in the exchange to show it had happened.
